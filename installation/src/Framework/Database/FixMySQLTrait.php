...
/**
 * Trait to normalise the database connection information to a MySQL database.
 *
 * @since  10.0
 */
trait FixMySQLTrait
{
	/**
	 * Normalise the connection information to MySQL.
	 *
	 ...
	 *
	 * @param   string|null  $host    The hostname. Can contain legacy hostname:port or hostname:sc=ocket definitions.
	 * @param   int|null     $port    The port. Alternatively it can contain the path to the socket.
	 * @param   string|null  $socket  The path to the socket. You could abuse it to enter the port number. DON'T!
	 * @param   bool         $useSSL  Is the server connection encryption enabled?
	 *
	 * @return  void  All parameters are passed by reference.
	 * @since   10.0
	 */
	protected function fixHostnamePortSocket(?string &$host, ?int &$port, ?string &$socket, bool $useSSL = false): void
	{
	 ...
	 * @since   10.0
	 */
	protected function createMySQLHostname(?string $host, ?string $port, ?string $socket, bool $hasSSL = false): string
	{
