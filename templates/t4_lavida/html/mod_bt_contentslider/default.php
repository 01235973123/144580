...
<div id="btcontentslider<?php echo $module->id; ?>" style="display:none;width:<?php echo $moduleWidthWrapper;?>" class="bt-cs<?php echo $moduleclass_sfx? ' bt-cs'.$params->get('moduleclass_sfx'):'';?>">
	...
	<div class="bt-cs-control"><span class="prev"><i class="fa fa-chevron-left"></i></span><span class="next"><i class="fa fa-chevron-right"></i></span></div> 
	<?php endif; ?>
	<?php 
		$add_style = "";
		...
		<?php foreach( $list as $i => $row ): ?>
			<div class="bt-row <?php if($i==0) echo 'bt-row-first'; else if($i==count($list)-1) echo 'bt-row-last' ?>"  style="width:<?php echo $itemWidth;?>%" >
				...
				<?php if( $row->thumbnail && $align_image != "center"): ?>
                    ...
                    <a  class="info" href="<?php echo $modal?$row->mainImage:$row->link;?>" title="<?php echo $row->title;?>"><i class="icon-edit"></i></a>
				<?php endif ; ?>
				...
				<?php if($show_category_name_as_link) : ?>
					...
					<img src="<?php echo $row->thumbnail; ?>" alt="<?php echo $row->title?>"  style="width:<?php echo $thumbWidth ;?>px;" title="<?php echo $row->title?>" />
