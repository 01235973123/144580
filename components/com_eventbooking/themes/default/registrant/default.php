	...
	<form action="<?php echo Route::_('index.php?option=com_eventbooking&view=registrants&Itemid=' . $this->Itemid); ?>" method="post" name="adminForm" id="adminForm" class="<?php echo $formHorizontalClass; ?>">
		...
		<div class="<?php echo $controlGroupClass; ?>">
			...
			<div class="<?php echo $controlsClass; ?>">
				...
				{
				?>
					<input class="form-control validate[required,custom[number]]" type="text" name="number_registrants"
				...
				<div class="<?php echo $controlGroupClass; ?>">
					...
					<div class="<?php echo $controlsClass; ?>">
						...
						if ($available > 0 || $quantity > 0)
						{
							...
							{
								echo HTMLHelper::_('select.integerlist', 0, $available, 1, $fieldName, 'class="ticket_type_quantity form-select input-small"', $quantity);
							}
							else
							{
								echo $quantity;
		...
		<div class="<?php echo $controlGroupClass; ?>">
			...
			<div class="<?php echo $controlsClass; ?>">
				...
				if ($this->canChangeStatus)
				{
					$input = '<input type="text" class="form-control" name="total_amount" value="' . ($this->item->total_amount > 0 ? round($this->item->total_amount, $numberDecimals) : '') . '" size="7" />';

					echo $bootstrapHelper->getAddon($input, $this->config->currency_symbol, $this->config->currency_position);
				}
				else
				{
		...
		if ($this->item->discount_amount > 0 || $this->item->late_fee > 0 || $this->item->tax_amount > 0 || $this->canChangeStatus || empty($this->item->id))
		{
			...
			?>
				<div class="<?php echo $controlGroupClass; ?>">
					...
					<div class="<?php echo $controlsClass; ?>">
						...
						if ($this->canChangeStatus)
						{
							$input = '<input type="text" class="form-control" name="discount_amount" value="' . ($this->item->discount_amount > 0 ? round($this->item->discount_amount, $numberDecimals) : '') . '" size="7" />';
			...
			?>
				<div class="<?php echo $controlGroupClass; ?>">
					...
					<div class="<?php echo $controlsClass; ?>">
						...
						if ($this->canChangeStatus)
						{
							$input = '<input type="text" class="form-control" name="late_fee" value="' . ($this->item->late_fee > 0 ? round($this->item->late_fee, $numberDecimals) : '') . '" size="7" />';
			...
			?>
				<div class="<?php echo $controlGroupClass; ?>">
					...
					<div class="<?php echo $controlsClass; ?>">
						...
						if ($this->canChangeStatus)
						{
							$input = '<input type="text" class="form-control" name="tax_amount" value="' . ($this->item->tax_amount > 0 ? round($this->item->tax_amount, $numberDecimals) : '') . '" size="7" />';
			...
			<div class="<?php echo $controlGroupClass; ?>">
				...
				<div class="<?php echo $controlsClass; ?>">
					...
					if ($this->canChangeStatus)
					{
						$input = '<input type="text" class="form-control" name="amount" value="' . ($this->item->amount > 0 ? round($this->item->amount, $numberDecimals) : '') . '" size="7" />';
		...
		if ($this->item->id > 0 && in_array($this->item->published, [3,4]))
		{
			$showDepositAmountFields = false;
		}
		else
		{
			$showDepositAmountFields = true;
		}

		if ($showDepositAmountFields && ($this->item->deposit_amount > 0 || ($this->config->activate_deposit_feature && $this->canChangeStatus)))
		{
		?>
			<div class="<?php echo $controlGroupClass; ?>">
				<div class="<?php echo $controlLabelClass; ?>">
				...
				<div class="<?php echo $controlsClass; ?>">
					...
					if ($this->canChangeStatus)
					{
						$input = '<input type="text" class="form-control" name="deposit_amount" value="' . round($this->item->deposit_amount,
		...
		<?php
			for ($i = 0, $n = count($this->rowMembers); $i < $n; $i++)
			{
				...
				// Handle custom fields depend on ticket types on add/edit event scree
