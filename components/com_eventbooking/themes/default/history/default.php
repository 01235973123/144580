...
?>
<div id="eb-registration-history-page" class="eb-container eb-joomla4-container">
	<?php
	...
	{
	?>
		<div class="eb-description eb-registration-history-intro-text"><?php echo $this->params->get('intro_text');?></div>
	<?php
	}
	?>
	...
	<?php
		...
		?>
			...
			<thead>
				<tr>
					<?php
					if ($this->config->get('history_show_first_name', 1))
					{
						$cols++;
					?>
						<th>
					...
					if ($showLastName && $this->config->get('history_show_last_name', 1))
					{
						$cols++;
					?>
						<th>
					...
					<?php
						...
						if ($this->config->activate_invoice_feature && $this->params->get('show_invoice_column', 1))
						{
							$cols++;
						?>
							<th class="center">
						...
						if ($this->showDownloadTicket && $this->params->get('show_ticket_column', 1))
						{
							$cols++;
						?>
							<th class="center">
						...
						if ($this->showDownloadCertificate && $this->params->get('show_certificate_column', 1))
						{
							$cols++;
						?>
							<th class="center">
				...
				<tr>
					...
					?>
						<td style="text-align: right;" class="tdno<?php echo $i; ?>" data-content="<?php echo Text::_('EB_DUE_AMOUNT'); ?>">
							...
							if ($row->payment_status != 1 && $row->published != 2)
							{
								...
								if ($hasOnlinePaymentMethods && $row->group_id == 0)
								{
								?>
	                ...
	                <td class="tdno<?php echo $i; ?> center" data-content="<?php echo Text::_('EB_REGISTRATION_STATUS'); ?>">
						...
						switch($row->published)
						{
							case 0 :
								...
								if ($row->group_id == 0 && $hasOnlinePaymentMethods && strpos($row->payment_method, 'os_offline') !== false)
								{
								?>
					...
					?>
	                    <td class="tdno<?php echo $i; ?> center" data-content="<?php echo Text::_('EB_INVOICE_NUMBER'); ?>">
							...
							{
							?>
	                            <a href="<?php echo Route::_('index.php?option=com_eventbooking&task=registrant.download_invoice&id=' . ($row->cart_id ?: ($row->group_id ?: $row->id)) . '&Itemid=' . $this->Itemid); ?>" title="<?php echo Text::_('EB_DOWNLOAD'); ?>"><?php echo EventbookingHelper::callOverridableHelperMethod('Helper', 'formatInvoiceNumber', [$row->invoice_number, $this->config, $row]) ; ?></a>
	                        <?php
							}
							?>
	                    </td>
