...
$checkoutUrl = Route::_(
	'index.php?option=com_eventbooking&task=view_checkout&Itemid=' . $this->Itemid . EventbookingHelper::addTimeToUrl(),
	false
);
$btnClass    = $this->bootstrapHelper->getClassMapping('btn');

if ($addMoreEventsUrl = EventbookingHelperCart::getAddMoreEventsUrl())
{
	$addMoreEventsButtonClick = "javascript:gotoEventsPage('" . $addMoreEventsUrl . "');";
}
else
{
	$addMoreEventsButtonClick = 'javascript:closeCartPopup();';
}
?>
...
?>
	<form method="post" name="adminForm" id="adminForm" action="index.php">
					...
					<tr>
						...
						<td class="col_quantity">
							<div class="btn-wrapper input-append<?php echo $inputGroupClass; ?>">
								<input id="quantity" type="number"<?php if ($item->maxQuantity != -1) echo ' max="' . $item->maxQuantity . '"'; ?> min="0" class="form-control input-mini quantity_box" size="3" value="<?php echo $item->quantity ; ?>" name="quantity[]" <?php echo $readOnly ; ?> onchange="updateCart(<?php echo (int) $this->Itemid; ?>);" />
		...
		<div style="text-align: center;" class="form-actions">
			<div class="controls">
				<button onclick="<?php echo $addMoreEventsButtonClick; ?>" id="add_more_item" class="<?php echo $btnClass; ?> btn-success" type="button">
