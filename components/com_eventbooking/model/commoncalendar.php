...
class EventbookingModelCommoncalendar extends RADModel
{
	...
	protected function buildQuery()
	{
		...
		{
			$query->where(
				'a.id IN (SELECT event_id FROM #__eb_event_categories WHERE category_id IN (' . implode(
					',',
					$allCategoryIds
				) . '))'
			);
		...
		{
			$query->where(
				'a.id NOT IN (SELECT event_id FROM #__eb_event_categories WHERE category_id IN (' . implode(
					',',
					$excludeCategoryIds
				) . '))'
			);
		...
		{
			$query->where(
				'a.id IN (SELECT event_id FROM #__eb_event_speakers WHERE speaker_id IN (' . implode(
					',',
					$speakerIds
				) . '))'
			);
		...
		{
			$query->where(
				' a.location_id IN (SELECT id FROM #__eb_locations WHERE LOWER(`city`) = ' . $db->quote(
