...
class EventbookingModelList extends RADModelList
{
	...
	public function __construct($config = [])
	{
		...
		if (is_file(__DIR__ . '/custom_event_fields.php'))
		{
	...
	protected function buildQueryColumns(DatabaseQuery $query)
	{
		...
		$query->select($fieldsToSelect)
			...
			->select(
				$db->quoteName(['c.name' . $fieldSuffix, 'c.alias' . $fieldSuffix], ['location_name', 'location_alias'])
			)
	...
	protected function buildQueryWhere(DatabaseQuery $query)
	{
		...
		{
			$query->where(
				'tbl.id NOT IN (SELECT event_id FROM #__eb_event_categories WHERE category_id IN (' . implode(
					',',
					$excludeCategoryIds
				) . '))'
			);
		...
		{
			$query->where(
				'tbl.id IN (SELECT event_id FROM #__eb_event_speakers WHERE speaker_id IN (' . implode(
					',',
					$speakerIds
				) . '))'
			);
		...
		{
			$query->where(
				' tbl.location_id IN (SELECT id FROM #__eb_locations WHERE LOWER(`city`) = ' . $db->quote(
					StringHelper::strtolower($filterCity)
				) . ')'
			);
		}
	...
	protected function applyHideFutureEventsFilter($query)
	{
		...
		$query->where(
			"(tbl.event_date <  $nowDate AND (tbl.event_end_date = $nullDate OR tbl.event_end_date < $nowDate))"
		);
	}
}
