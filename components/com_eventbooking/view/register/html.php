...
class EventbookingViewRegisterHtml extends EventbookingViewRegisterBase
{
	...
	protected function displayIndividualRegistrationForm($event, $input)
	{
		...
		$collectMembersInformation = $event->has_multiple_ticket_types
			&& EventbookingHelperRegistration::isCollectMemberInformationForTicketTypes($event);

		/**
		 * For event with ticket types and collect members information set to Yes, treat
		 * the billing information here as group billing
		 */
		...
		$this->collectMembersInformation = $collectMembersInformation;
	...
	protected function displayCart()
	{
		...
		$paymentMethod = $input->post->getString(
			'payment_method',
			EventbookingHelperPayments::getDefautPaymentMethod()
		);
		$form->handleFieldsDependOnPaymentMethod($paymentMethod);
	...
	protected function generateBreadcrumb($event, $layout)
	{
		...
		if (in_array($menuView, ['calendar', 'fullcalendar', 'categories', 'category', 'upcomingevents']))
		{
			$pathway->addItem(
				$event->title,
				Route::_(EventbookingHelperRoute::getEventRoute($event->id, 0, $app->getInput()->getInt('Itemid')))
			);
		}
				...
				if ($preventDuplicateRegistration && $user->id > 0)
				{
					...
					if ($total)
					{
						...
						if (!$this->input->getInt('hmvc_call'))
						{
							$app->redirect(
								Route::_(
									EventbookingHelperRoute::getEventRoute($event->id, $event->main_category_id),
									false
								)
							);
						}
	...
	protected function setPageTitle($event, $layout)
	{
		...
		if (!$pageTitle)
		{
			...
			$pageTitle = str_replace(
				'[EVENT_DATE]',
				HTMLHelper::_('date', $event->event_date, $config->event_date_format, null),
				$pageTitle
			);
		}
	...
	protected function setRegistrationFormHeadingAndMessage($layout)
	{
		if ($this->waitingList)
		{
			...
			if ($this->fieldSuffix && EventbookingHelper::isValidMessage(
					$this->message->{'waitinglist_form_message' . $this->fieldSuffix}
				))
			{
	...
	{
		if ($this->fieldSuffix && EventbookingHelper::isValidMessage(
				$this->message->{'registration_form_message' . $this->fieldSuffix}
			))
		{
	...
	protected function addGroupRegistrationJSVariables()
	{
		...
		Factory::getApplication()->getDocument()
			...
			->useScript('core')
			->addInlineScript(
				'var returnUrl = "' . base64_encode(Uri::getInstance()->toString() . '#group_billing') . '";'
			);
		...
		$billingFields = EventbookingHelperRegistration::getFormFields(
			$this->event->id,
			1,
			null,
			null,
