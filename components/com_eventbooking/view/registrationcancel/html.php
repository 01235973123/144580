...
class EventbookingViewRegistrationcancelHtml extends RADViewHtml
{
	...
	public function display()
	{
		...
		if ($rowRegistrant->published == 4)
		{
			if ($fieldSuffix && EventbookingHelper::isValidMessage(
					$message->{'waiting_list_cancel_complete_message' . $fieldSuffix}
				))
			{
		...
		{
			if ($fieldSuffix && EventbookingHelper::isValidMessage(
					$message->{'registration_cancel_message_paid' . $fieldSuffix}
				))
			{
		...
		{
			if ($fieldSuffix && EventbookingHelper::isValidMessage(
					$message->{'registration_cancel_message_free' . $fieldSuffix}
				))
			{
	...
	protected function displayConfirmationForm()
	{
		...
		if (!EventbookingHelperRegistration::canCancelRegistrationNow($row))
		{
			...
			{
				$cancelBeforeDate = Factory::getDate(
					$row->cancel_before_date,
