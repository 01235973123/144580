...
class EventbookingViewPaymentHtml extends RADViewHtml
{
	...
	public function display()
	{
		...
		$fees = EventbookingHelper::callOverridableHelperMethod(
			...
			[$rowRegistrant, $this->paymentMethod]
		);
	...
	protected function displayRegistrationPayment()
	{
		...
		if (property_exists(
				$event,
				'validate_capacity_for_waiting_list_payment'
			) && $event->validate_capacity_for_waiting_list_payment !== '')
		{
		...
		Factory::getApplication()->getDocument()->addScriptOptions(
			'currencyCode',
			$event->currency_code ?: $config->currency_code
		);
