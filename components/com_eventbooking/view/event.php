...
trait EventbookingViewEvent
{
	...
	/**
	 * Event data stored in params column
	 *
	 * @var Registry
	 */
	...
	public function buildFormData($item, $categories, $locations)
	{
		...
		$this->lists['main_category_id'] = HTMLHelper::_('select.genericlist', $options, 'main_category_id', [
			...
			'option.text'        => 'text',
			'option.value'       => 'value',
			'list.attr'          => 'class="validate[required] form-select"',
		...
		$this->lists['category_id'] = HTMLHelper::_('select.genericlist', $options, 'category_id[]', [
			...
			'option.text'        => 'text',
			'option.value'       => 'value',
			'list.attr'          => 'class="form-select" multiple',
		...
		$this->lists['access']              = HTMLHelper::_(
			'access.level',
			'access',
			$item->access,
			'class="form-select"',
		...
		else
		{
			...
			$selectedMinute = $this->input->post->getInt(
				'event_date_minute',
				$config->get('default_event_date_minute', 0)
			);
		}
		...
		{
			$selectedHour   = $this->input->post->getInt(
				'event_end_date_hour',
				$config->get('default_event_end_date_hour', 0)
			);
			$selectedMinute = $this->input->post->getInt(
				'event_end_date_minute',
				$config->get('default_event_end_date_minute', 0)
			);
		}
		...
		$options[] = HTMLHelper::_(
			'select.option',
			'',
			Text::_('JGLOBAL_USE_GLOBAL') . ' (' . $config->currency_symbol . ')'
		);
		...
		$this->lists['send_ics_file'] = HTMLHelper::_(
			'select.genericlist',
			$options,
			'options[send_ics_file]',
			'class="form-select"',
			'value',
			'text',
			$params->get('send_ics_file', '')
		);
		...
		$this->lists['deposit_amount_apply_for'] = HTMLHelper::_(
			'select.genericlist',
			$options,
			'options[deposit_amount_apply_for]',
			'class="form-select"',
			'value',
			'text',
			$params->get('deposit_amount_apply_for', 0)
		);
		...
		$this->lists['simply_registration_process'] = HTMLHelper::_(
			'select.genericlist',
			$options,
			'options[simply_registration_process]',
			'class="form-select"',
			'value',
			'text',
			$params->get('simply_registration_process', '')
		);

		if ($config->activate_invoice_feature)
		{
			$this->lists['enable_invoice'] = EventbookingHelperHtml::getBooleanInput(
				'options[enable_invoice]',
				$params->get('enable_invoice', 1)
			);
		}
