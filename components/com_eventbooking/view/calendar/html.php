...
class EventbookingViewCalendarHtml extends RADViewHtml
{
	...
	public function display()
	{
		...
		$this->searchMonth = HTMLHelper::_(
			'select.genericlist',
			$options,
			'month',
			'class="input-medium form-select w-auto" onchange="submit();" ',
			'value',
			'text',
			(int) $month
		);
	...
	protected function getCalendarEventAttributes($event, $params)
	{
		...
		{
			if (is_file(JPATH_ROOT . '/media/com_eventbooking/assets/images/custom_calendar_event.png'))
			{
				$thumbSource = $rootUri . '/media/com_eventbooking/assets/images/custom_calendar_event.png';
			}
			else
			{
