...
trait EventbookingViewRegistrant
{
	...
	public function prepareViewData()
	{
		...
		if (count($rowMembers))
		{
			$this->memberFormFields = EventbookingHelperRegistration::getFormFields(
				$this->item->event_id,
				2,
				$this->item->language
			);
		...
		if ($this->item->id && ($config->hide_past_events_from_events_dropdown || $config->get(
					'hide_unpublished_events_from_events_dropdown',
					1
				)))
		{
			$eventExists = false;
