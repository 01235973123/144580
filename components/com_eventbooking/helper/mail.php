...
class EventbookingHelperMail
{
	...
	public static function sendEmails($row, $config)
	{
		...
		$event       = EventbookingHelperDatabase::getEvent($row->event_id, null, $fieldSuffix, true);
		...
		EventbookingHelper::setEventStringsDataFromCategory(
			$event,
			$category,
			['user_email_subject', 'notification_emails'],
			$fieldSuffix
		);
	 ...
	 */
	public static function sendRegistrationEmailToRegistrant(
		$mailer,
		$row,
		$rowMembers,
		$replaces,
		...
		if (!$row->published && str_contains($row->payment_method, 'os_offline'))
		{
			...
			elseif ($offlineSuffix && EventbookingHelper::isValidMessage(
					$message->{'user_email_body_offline' . $offlineSuffix}
				))
			{
				$body = $message->{'user_email_body_offline' . $offlineSuffix};
			}
			elseif ($fieldSuffix && EventbookingHelper::isValidMessage(
					$event->{'user_email_body_offline' . $fieldSuffix}
				))
			{
	...
	protected static function sendRegistrationEmailToGroupMembers($mailer, $row, $rowMembers, $replaces, $attachments)
	{
		...
		elseif ($fieldSuffix && EventbookingHelper::isValidMessage(
				$message->{'group_member_email_body' . $fieldSuffix}
			))
		{
		...
		{
			$memberReplaces[$tag] = $replaces[$tag];
		}
		...
		foreach ($rowMembers as $rowMember)
		{
			...
			$groupMemberEmailSubject = EventbookingHelper::replaceCaseInsensitiveTags(
				$groupMemberEmailSubject,
				$memberReplaces
			);
			$groupMemberEmailBody    = EventbookingHelper::replaceCaseInsensitiveTags(
			...
			{
				$ticketNumber   = EventbookingHelperTicket::formatTicketNumber(
					$event->ticket_prefix,
					$rowMember->ticket_number,
					$config
				);
	 ...
	 * @param   string                       $invoiceFilePath
	 */
	protected static function sendRegistrationEmailToAdmin(
		$mailer,
		$row,
		$form,
		$replaces,
		...
		{
			$replaces['registration_detail'] = EventbookingHelperRegistration::getEmailContent(
				$config,
				$row,
				true,
				$form,
	...
	/**
	 * Method to resend email to group member
	 *
	 * @param   EventbookingTableRegistrant  $rowGroupBilling
	 * @param   EventbookingTableRegistrant  $rowGroupMember
	 *
	 * @return void
	 */
	public static function resendGroupMemberEmail($rowGroupBilling, $rowGroupMember)
	{
		if (!MailHelper::isEmailAddress($rowGroupMember->email))
		{
			return;
		}

		$config = EventbookingHelper::getConfig();
		$event  = EventbookingHelperDatabase::getEvent($rowGroupBilling->event_id);

		$mailer   = self::getMailer($config, $event);
		$replaces = EventbookingHelperRegistration::getRegistrationReplaces($rowGroupBilling, $event);

		static::sendRegistrationEmailToGroupMembers($mailer, $rowGroupBilling, [$rowGroupMember], $replaces, []);
	}
	...
	public static function sendRegistrationApprovedEmail($row, $config)
	{
		...
		if (isset($event->registration_approved_email_subject) && strlen(
				trim($event->registration_approved_email_subject)
			))
		{
		...
		elseif ($fieldSuffix && EventbookingHelper::isValidMessage(
				$message->{'registration_approved_email_body' . $fieldSuffix}
			))
		{
			$body = $message->{'registration_approved_email_body' . $fieldSuffix};
		}
		elseif (isset($event->registration_approved_email_body) && EventbookingHelper::isValidMessage(
				$event->registration_approved_email_body
			))
		{
		...
		if (str_contains($body, '[QRCODE]'))
		{
			...
			$imgTag = '<img src="' . EventbookingHelper::getSiteUrl(
				) . 'media/com_eventbooking/qrcodes/' . $row->id . '.png" border="0" />';
			$body   = str_ireplace('[QRCODE]', $imgTag, $body);
		}

		if ($config->activate_invoice_feature
			&& ($config->send_invoice_to_customer || $config->send_invoice_to_admin)
			&& $config->get('send_invoice_when_registration_approved', 1)
			&& $row->invoice_number && !$row->group_id)
		{
			$invoiceFilePath = EventbookingHelper::callOverridableHelperMethod('Helper', 'generateInvoicePDF', [$row]);
		...
		if ($fieldSuffix && EventbookingHelper::isValidMessage(
				$message->{'admin_registration_approved_email_body' . $fieldSuffix}
			))
		{
	...
	public static function sendRegistrationCancelledEmail($row, $config)
	{
		...
		if ($fieldSuffix && EventbookingHelper::isValidMessage(
				$message->{'user_registration_cancel_message' . $fieldSuffix}
			))
		{
		...
		if ($fieldSuffix && EventbookingHelper::isValidMessage(
				$message->{'admin_cancel_registration_notification_email_body' . $fieldSuffix}
			))
		{
	...
	public static function sendWaitinglistEmail($row, $config)
	{
		...
		if ($fieldSuffix && EventbookingHelper::isValidMessage(
				$message->{'watinglist_confirmation_body' . $fieldSuffix}
			))
		{
	...
	public static function sendWaitingListNotificationEmail($row, $config)
	{
		...
		if (count($registrants))
		{
			...
			foreach ($registrants as $registrant)
			{
				...
				if ($fieldSuffix && EventbookingHelper::isValidMessage(
						$message->{'registrant_waitinglist_notification_body' . $fieldSuffix}
					))
				{
	...
	public static function sendDepositPaymentEmail($row, $config)
	{
		...
		if (MailHelper::isEmailAddress($row->email))
		{
			...
			if ($fieldSuffix && EventbookingHelper::isValidMessage(
					$message->{'deposit_payment_user_email_body' . $fieldSuffix}
				))
			{
			...
			if ($row->ticket_code)
			{
				$ticketFilePath = EventbookingHelper::callOverridableHelperMethod(
					'Ticket',
					'generateTicketsPDF',
					[$row, $config]
				);
				...
				if (!$ticketFilePath)
				{
					$ticketFilePath = JPATH_ROOT . '/media/com_eventbooking/tickets/ticket_' . str_pad(
							$row->id,
							5,
							'0',
							STR_PAD_LEFT
	...
	public static function sendNewEventNotificationEmail($row, $config)
	{
		...
		$replaces = array_merge($replaces, [
			...
			'event_link' => Uri::root(
				) . 'index.php?option=com_eventbooking&view=event&layout=form&id=' . $row->id . '&Itemid=' . $Itemid,
		]);
		...
		if ($fieldSuffix && EventbookingHelper::isValidMessage(
				$message->{'submit_event_user_email_body' . $fieldSuffix}
			))
		{
		...
		$replaces['event_link'] = Uri::root(
			) . 'administrator/index.php?option=com_eventbooking&view=event&id=' . $row->id;
	...
	public static function sendReminderEmails($params, $reminderNumber)
	{
		...
		foreach ($rows as $row)
		{
			...
			elseif ($fieldSuffix && EventbookingHelper::isValidMessage(
					$message->{$reminderEmailBodyField . $fieldSuffix}
				))
			{
			...
			$replaces = EventbookingHelperRegistration::getRegistrationReplaces($row, null, -1);
			...
			$sendCertificate = EventbookingHelperCertificate::shouldSendCertificateInReminderEmail(
				$row,
				$params,
				$reminderNumber
			);
	...
	public static function sendDepositReminder($numberDays, $numberEmailSendEachTime = 0, $bccEmail = null)
	{
		...
		foreach ($rows as $row)
		{
			...
			if ($fieldSuffix && EventbookingHelper::isValidMessage(
					$message->{'deposit_payment_reminder_email_body' . $fieldSuffix}
				))
			{
			...
			static::send(
				...
				$logEmails,
				self::SEND_TO_REGISTRANT,
				'deposit_payment_reminder_emails'
			);
			$mailer->clearAddresses();
	...
	public static function sendOfflinePaymentReminder($numberDaysToSendReminder, $numberRegistrants, $params)
	{
		...
		foreach ($rows as $row)
		{
			...
			static::send(
				...
				$logEmails,
				self::SEND_TO_ADMIN,
				'offline_payment_reminder_emails'
			);
			$mailer->clearAddresses();
	...
	public static function sendEventCancelEmails($rows)
	{
		...
		foreach ($rows as $row)
		{
			...
			if ($fieldSuffix && EventbookingHelper::isValidMessage(
					$message->{'event_cancel_email_body' . $fieldSuffix}
				))
			{
	...
	protected static function addRegistrationIcs($mailer, $row, $event, $config)
	{
		...
		if ($config->multiple_booking)
		{
			...
			$query = $db->getQuery(true)
				->select(
					'a.id, a.title, a.event_date, a.event_end_date, a.params, a.short_description, l.lat, l.long, l.address AS location_address'
				)
		...
		else
		{
			...
			$mailer->addAttachment(
				$fileName,
				ApplicationHelper::stringURLSafe($event->title) . '.ics',
				'base64',
				'text/calendar'
			);
		}
	...
	public static function send($mailer, $emails, $subject, $body, $logEmails = false, $sentTo = 0, $emailType = '')
	{
		...
		$emailBody = EventbookingHelperHtml::loadSharedLayout(
	...
	public static function sendRequestPaymentEmail($row, $config)
	{
		...
		if ($row->published == 0)
		{
			...
			if ($fieldSuffix && EventbookingHelper::isValidMessage(
					$message->{'request_payment_email_body_pdr' . $fieldSuffix}
				))
			{
			...
			elseif ($fieldSuffix && EventbookingHelper::isValidMessage(
					$message->{'request_payment_email_body' . $fieldSuffix}
				))
			{
	...
	public static function sendCertificateEmail($row, $config)
	{
		...
		{
			throw new Exception(
				'Email subject could not be empty. Go to Events Booking -> Emails & Messages and setup Certificate email subject'
			);
		}
		...
		{
			throw new Exception(
				'Email message could not be empty. Go to Events Booking -> Emails & Messages and setup Certificate email body'
			);
		}
	...
	public static function sendUserCancelRegistrationEmail($row, $config)
	{
		...
		if ($fieldSuffix && EventbookingHelper::isValidMessage(
				$message->{$keyPrefix . '_confirmation_email_body' . $fieldSuffix}
			))
		{
