...
abstract class EventbookingHelperJquery
{
	...
	public static function colorbox(
		...
		if (!$loaded)
		{
			...
			$wa->registerAndUseStyle(
				'com_eventbooking.colorbox',
				'media/com_eventbooking/assets/js/colorbox/colorbox.min.css'
			)
				->registerAndUseScript(
					'com_eventbooking.colorbox',
					'media/com_eventbooking/assets/js/colorbox/jquery.colorbox.min.js'
				);
		...
		if ($class == 'a.eb-modal')
		{
			...
			$script  = 'Eb.jQuery(document).ready(function($){$("' . $class . '").colorbox(' . self::getJSObject(
					$options
				) . ');});';
		}
		else
	...
	public static function getJSObject(array $array = [])
	{
		...
		foreach ((array) $array as $k => $v)
		{
			...
			{
				$object .= ' ' . $k . ': ';
				$object .= (is_numeric($v) || str_starts_with($v, '\\')) ? (is_numeric($v)) ? $v : substr(
					$v,
					1
				) : "'" . $v . "'";
				$object .= ',';
	...
	public static function validateForm()
	{
		...
		if (!$loaded)
		{
			...
			{
				$eventId = Factory::getApplication()->getInput()->getInt('event_id', 0);
			}
			...
			if (is_file(JPATH_ROOT . '/media/com_eventbooking/js/custom_validation_rules.js'))
			{
				$wa->registerAndUseScript(
					'com_eventbooking.validationEngine.custom_rules',
					'media/com_eventbooking/js/custom_validation_rules.js'
				);
			}
