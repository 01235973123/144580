...
class EventbookingHelperCart
{
	...
	/**
	 * Get add more events URL
	 *
	 * @return string
	 */
	public static function getAddMoreEventsUrl()
	{
		$addMoreEventsUrl       = '';
		$config                 = EventbookingHelper::getConfig();
		$addMoreEventMenuItemId = $config->add_more_events_menu_item_id;

		if ($addMoreEventMenuItemId > 0)
		{
			if (Multilanguage::isEnabled())
			{
				...
				{
					$addMoreEventMenuItemId = $langAssociations[$langCode]->id;
				}
			}

			if ($item = Factory::getApplication()->getMenu()->getItem($addMoreEventMenuItemId))
			{
				$addMoreEventsUrl = 'index.php?Itemid=' . $addMoreEventMenuItemId;

				if (Multilanguage::isEnabled() && $item->language && $item->language != '*')
				{
					$addMoreEventsUrl .= '&lang=' . $item->language;
				}

				return Route::_($addMoreEventsUrl);
			}
		}

		return $addMoreEventsUrl;
	}
}
