...
class EventbookingHelperRegistration
{
	...
	public static function isCouponAvailableForEvent($event, $registrationType = 0)
	{
		...
		$query = EventbookingHelper::callOverridableHelperMethod(
			'Registration',
			'getCouponQuery',
	...
	public static function getFormFields(
		...
		{
			return EventbookingHelperOverrideRegistration::getFormFields(
				$eventId,
				$registrationType,
				$activeLanguage,
				$userId
		...
		if (empty($cache[$cacheKey]))
		{
			...
			if ($registrationType == 4)
			{
				...
				if ($filterByEvents)
				{
					$whereOrs[] = 'id IN (SELECT field_id FROM #__eb_field_events AS fv WHERE fv.event_id IN (' . implode(
							',',
							$items
						) . '))';
			...
			else
			{
				...
				if ($filterByEvents)
				{
					...
					$query->where(
						'id NOT IN (SELECT field_id FROM #__eb_field_events WHERE event_id = ' . $negEventId . ')'
					);
				}
	 ...
	 * @return array
	 */
	public static function getRegistrationReplaces(
		$row,
		$rowEvent = null,
		$userId = 0,
		...
		{
			$rowFields = EventbookingHelperRegistration::getFormFields(
				$row->id,
				4,
				$row->language,
				$userId,
	...
	public static function getCouponQuery($couponCode, $event, $user)
	{
		...
		if (count($categoryIds) > 0)
		{
			$whereOrs[] = 'id IN (SELECT coupon_id FROM #__eb_coupon_categories AS ec WHERE ec.category_id IN (' . implode(
					',',
					$categoryIds
				) . '))';
		}
	...
	public static function calculateIndividualRegistrationFees($event, $form, $data, $config, $paymentMethod = null)
	{
		...
		else
		{
			EventbookingHelper::callOverridableHelperMethod(
				'Data',
				'calculateEventsDiscountedPriceForUser',
				[[$event], $user]
			);
		}
		...
		{
			$feeCalculationTags = array_merge(
				$feeCalculationTags,
				static::getFeeCalculationTagsFromEventFields($event)
			);
		}
		...
		if ($event->has_multiple_ticket_types)
		{
			...
			if ($collectMembersInformation)
			{
				...
				$fees['tickets_members'] = EventbookingHelperHtml::loadCommonLayout(
					'common/tmpl/tickets_members.php',
					$ticketsMembersData
				);
			}
		}
		...
		if ($user->id)
		{
			$discountAmount += static::calculateMemberDiscountForIndividualRegistration(
				$user,
				$event,
				$totalAmount,
				$fees
	...
	public static function getCouponForIndividualRegistration($couponCode, $event, $user)
	{
		...
		$couponQuery = EventbookingHelper::callOverridableHelperMethod(
			'Registration',
			'getCouponQuery',
			[$couponCode, $event, $user]
		);
	...
	public static function calculateGroupRegistrationFees($event, $form, $data, $config, $paymentMethod = null)
	{
		...
		// Late Fee
		$lateFee = static::calculateLateFeeForGroupRegistration(
			$event,
			$numberRegistrants,
			$totalAmount,
			$membersTotalAmount,
	 ...
	 * @return float
	 */
	protected static function calculateLateFeeForGroupRegistration(
		$event,
		$numberRegistrants,
		$totalAmount,
		$membersTotalAmount,
	 ...
	 * @return array
	 */
	public static function calculateCartRegistrationFee(
		$cart,
		$form,
		$data,
		$config,
		...
		$replaces['CART_AMOUNT']             = $cartAmount;
		$totalAmount                         = 0;
		$discountAmount                      = 0;
		$lateFee                             = 0;
		$taxAmount                           = 0;
		...
		for ($i = 0, $n = count($items); $i < $n; $i++)
		{
			...
			$registrantDepositAmount = static::calculateRegistrantDepositAmountForCart(
				$event,
				$registrantAmount,
				$quantity,
				$paymentType,
			...
			if ($collectRecordsData)
			{
				...
				$recordsData[$eventId]['payment_processing_fee'] = round(
					$registrantPaymentProcessingFee,
					$numberDecimals
				);
	...
	protected static function calculateCartDiscountBundle($user, $items, $cartAmount, &$fees): void
	{
		...
		if ($largestDiscountBundle)
		{
			...
			{
				$fees['bundle_discount_amount'] += round(
					$cartAmount * $largestDiscountBundle->discount_amount / 100,
					$numberDecimals
				);
			}
		...
		else
		{
			$query->clear()
				...
				->where('published = 1')
				->where(
					'id IN (SELECT discount_id FROM #__eb_discount_events WHERE event_id IN (' . implode(
						',',
						$items
					) . '))'
				);
			...
			if (!empty($discountRules))
			{
				...
				foreach ($discountRules as $rule)
				{
					...
					if (!array_diff($eventIds, $registeredEventIds))
					{
						...
						{
							$fees['bundle_discount_amount'] += round(
								$cartAmount * $rule->discount_amount / 100,
								$numberDecimals
							);
						}
	 ...
	 * @return float
	 */
	protected static function calculateRegistrantLateFeeForCart(
		$event,
		$registrantTotalAmount,
		$quantity,
		$membersTotalAmount,
	 ...
	 * @return float
	 */
	protected static function calculateRegistrantDepositAmountForCart(
		$event,
		$registrantAmount,
		$quantity,
		$paymentType,
		...
		if ($userId)
		{
			...
			if (!count($data))
			{
				...
				$query = $db->getQuery(true)
					...
					->from('#__eb_registrants')
					->where(
						'user_id = ' . $userId . ' AND event_id = ' . $eventId . ' AND first_name != "" AND group_id = 0'
					)
					->order('id DESC');
				$db->setQuery($query, 0, 1);
	...
	public static function saveRegistration($data)
	{
		...
		if ($result === false)
		{
			...
			EventbookingHelper::logData(
				JPATH_ROOT . '/components/com_eventbooking/user_creation_error.txt',
				$logData,
				$model->getError()
			);
		}
	...
	public static function needInvoice($row)
	{
		...
		if (!$config->multiple_booking)
		{
			...
			if (!$params->get('enable_invoice', 1))
			{
				return false;
			}
		}
	...
	public static function buildDepositPaymentTags($row, $config)
	{
		...
		$replaces['AMOUNT']         = EventbookingHelper::formatCurrency(
	...
	public static function buildEventTags($event, $config, $row = null, $Itemid = 0)
	{
		...
		if ((int) $event->event_end_date)
		{
			...
			{
				$replaces['event_end_date'] = HTMLHelper::_(
					'date',
					$event->event_end_date,
					$config->event_date_format,
					null
				);
			}

			$replaces['event_end_date_date'] = HTMLHelper::_(
				'date',
				$event->event_end_date,
				$config->date_format,
				null
		...
		{
			$replaces['cancel_before_date'] = HTMLHelper::_(
				'date',
		...
		$replaces['individual_price'] = EventbookingHelper::formatCurrency(
		...
		{
			/* @var User $creator */
			$creator = Factory::getContainer()->get(UserFactoryInterface::class)->loadUserById(
	...
	public static function buildTags($row, $form, $event, $config, $loadCss = true)
	{
		...
		{
			$Itemid = EventbookingHelperRoute::getEventMenuId(
				$event->id,
		...
		if ($config->multiple_booking)
		{
			...
			$replaces['total_amount']                = EventbookingHelper::formatCurrency(
				$totalAmount,
				$config,
				$event->currency_symbol
			);
			...
			$replaces['tax_amount']                  = EventbookingHelper::formatCurrency(
				$taxAmount,
				$config,
				$event->currency_symbol
			);
			$replaces['discount_amount']             = EventbookingHelper::formatCurrency(
				$discountAmount,
				$config,
				$event->currency_symbol
			);
			$replaces['late_fee']                    = EventbookingHelper::formatCurrency(
				$lateFee,
				$config,
				$event->currency_symbol
			);
		...
		else
		{
			...
			$replaces['total_amount_without_currency']           = EventbookingHelper::formatAmount(
				$row->total_amount,
				$config
			);
			$replaces['tax_amount_without_currency']             = EventbookingHelper::formatAmount(
				$row->tax_amount,
				$config
			);
			$replaces['discount_amount_without_currency']        = EventbookingHelper::formatAmount(
				$row->discount_amount,
				$config
			);
			$replaces['late_fee_without_currency']               = EventbookingHelper::formatAmount(
				$row->late_fee,
				$config
			);
			$replaces['payment_processing_fee_without_currency'] = EventbookingHelper::formatAmount(
				$row->payment_processing_fee,
				$config
			);
			$replaces['amount_without_currency']                 = EventbookingHelper::formatAmount(
				$row->amount,
				$config
			);
			...
			$replaces['amt_payment_processing_fee'] = EventbookingHelper::roundAmount(
				$row->payment_processing_fee,
				$config
			);
			...
			else
			{
				...
				if ($row->published == 1)
				{
					$replaces['PAID_AMOUNT'] = EventbookingHelper::formatCurrency(
						$depositAmount,
						$config,
						$event->currency_symbol
					);
			...
			if ($row->published == 0)
			{
				...
				{
					$replaces['REGISTRATION_DUE_AMOUNT']  = EventbookingHelper::formatCurrency(
						$dueAmount,
						$config,
						$event->currency_symbol
					);
					$replaces['REGISTRATION_PAID_AMOUNT'] = EventbookingHelper::formatCurrency(
						$depositAmount,
						$config,
						$event->currency_symbol
					);
			...
			if ($event->has_multiple_ticket_types)
			{
				...
				{
					$ticketsOutput[]          = Text::_(
							$ticketTypes[$registrantTicket->ticket_type_id]->title
						) . ': ' . $registrantTicket->quantity;
					$replaces['TICKET_TYPE']  = Text::_($ticketTypes[$registrantTicket->ticket_type_id]->title);
					$replaces['TICKET_PRICE'] = EventbookingHelper::formatAmount(
						$ticketTypes[$registrantTicket->ticket_type_id]->price,
						$config
					);
				}
		...
		if ((int) $row->register_date)
		{
			...
			$replaces['register_date_time']         = HTMLHelper::_(
				'date',
				$row->register_date,
				$config->date_format . ' H:i:s'
			);
		...
		{
			$replaces['registration_cancel_date'] = HTMLHelper::_(
				'date',
		...
		{
			$replaces['ticket_number'] = EventbookingHelperTicket::formatTicketNumber(
				$event->ticket_prefix,
				$row->ticket_number,
				$config
			);
		...
		if ($row->is_group_billing && $collectMemberInformation)
		{
			...
			$db->setQuery($query);
			$rowMembers                = $db->loadObjectList();
			$memberFormFields          = EventbookingHelperRegistration::getFormFields(
				$row->event_id,
				2,
				$row->language,
				$row->user_id
	...
	public static function generateTicketNumberQrcode($ticketNumber)
	{
		static::generateQRCODEFile(
			$ticketNumber,
			JPATH_ROOT . '/media/com_eventbooking/qrcodes/' . $ticketNumber . '.png'
		);
	}
	...
	public static function processQRCODE($row, $output, $absolutePath = true)
	{
		...
		if (str_contains($output, '[QRCODE_REGISTRANT_DATA]'))
		{
			...
			self::generateQRCODEFile(
				$registrantQRCODEData,
				JPATH_ROOT . '/media/com_eventbooking/qrcodes/registrant_data_' . $row->id . '.png'
			);
	...
	public static function getRegistrantId($eventId)
	{
		...
		if ($cache === null)
		{
			...
			$query = $db->getQuery(true)
				...
				->where(
					'(published = 1 OR published = 3 OR (payment_method LIKE "os_offline%" AND published NOT IN (2, 4)))'
				)
				->order('id');
			$db->setQuery($query);
			$rows = $db->loadObjectList();
	 ...
	 * @return int
	 */
	public static function calculateEventTaxRate(
		$eventId,
		$country = '',
		$state = '',
		$vies = 2,
	...
	/**
	 * Check to see if collect members information is enabled for event
	 *
	 * @param   stdClass  $event
	 *
	 * @return bool
	 */
	public static function isCollectMemberInformationForTicketTypes($event): bool
	{
		if ($event->has_multiple_ticket_types)
		{
			$params = new Registry($event->params);

			return (bool) $params->get('ticket_types_collect_members_information');
		}
