...
use OSSolution\EventBooking\Admin\Event\PDF\BeforeGeneratePDF;
...
class EventbookingHelperPdf
{
	...
	public static function generatePDFFile($pages, $filePath, $options)
	{
		...
		$eventObj = new BeforeGeneratePDF(
			'onEBBeforeGeneratePDF',
			['pages' => $pages, 'filePath' => $filePath, 'options' => $options]
		);

		$results = Factory::getApplication()->triggerEvent('onEBBeforeGeneratePDF', $eventObj);
