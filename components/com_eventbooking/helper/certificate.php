...
class EventbookingHelperCertificate
{
	...
	public static function generateCertificates($rows, $config)
	{
		...
		foreach ($rows as $row)
		{
			...
			if ($row->is_group_billing && $collectMemberInformation)
			{
				...
				foreach ($rowMembers as $rowMember)
				{
					...
					$output = EventbookingHelper::callOverridableHelperMethod(
						'Html',
			...
			else
			{
				...
				$certificateLayout = EventbookingHelper::callOverridableHelperMethod(
					'Html',
					'processConditionalText',
					[$certificateLayout]
				);
		...
		{
			$row      = $rows[0];
			$fileName = EventbookingHelper::callOverridableHelperMethod(
					'Helper',
					'formatCertificateNumber',
					[$row->id, $config]
				) . '.pdf';
