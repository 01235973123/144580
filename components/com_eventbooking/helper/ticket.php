...
class EventbookingHelperTicket
{
	...
	public static function generateTicketsPDF($row, $config)
	{
		...
		$billingReplaces = EventbookingHelperRegistration::getRegistrationReplaces(
			$billingRow,
			$rowEvent,
			$currentUserId
		);
		...
		if ($row->is_group_billing)
		{
			foreach ($rowMembers as $rowMember)
			{
				...
				$replaces['ticket_number']     = self::formatTicketNumber(
					$rowEvent->ticket_prefix,
		...
		$filePath = JPATH_ROOT . '/media/com_eventbooking/tickets/ticket_' . str_pad(
				$row->id,
				5,
				'0',
				STR_PAD_LEFT
