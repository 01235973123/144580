...
class EventbookingHelperData
{
	...
	public static function calculateDiscount($rows)
	{
		...
		{
			if (EventbookingHelper::isMethodOverridden(
				'EventbookingHelperOverrideData',
				'calculateEventsDiscountedPrice'
			))
			{
	...
	public static function calculateEventsDiscountedPriceForUser($rows, $user = null)
	{
		...
		foreach ($rows as $row)
		{
			...
			{
				if (EventbookingHelper::isMethodOverridden(
					'EventbookingHelperOverrideRegistration',
	...
	public static function prepareCustomFieldsData($items, $checkShowField = true)
	{
		...
		foreach ($items as $item)
		{
			...
			foreach ($fieldsAssoc as $name => $field)
			{
				...
				$key                     = strtoupper($name);
				$item->short_description = str_ireplace(
					"[$key]",
					(string) $fieldValue,
	...
	public static function prepareDisplayData($events, $activeCategoryId, $config, $Itemid)
	{
		...
		{
			$event->can_register = EventbookingHelper::callOverridableHelperMethod(
				'Registration',
				'acceptRegistration',
				[$event]
			);
	 ...
	 * @return array
	 */
	public static function prepareRegistrantsExportData(
		$rows,
		$config,
		$rowFields,
		$fieldValues,
		...
		if ($config->activate_deposit_feature)
		{
			...
			if ($config->get('export_deposit_payment_status', 0))
			{
				$headers[] = Text::_('EB_DEPOSIT_PAYMENT_STATUS');
				$fields[]  = 'deposit_payment_status';
			}
		}
		...
		foreach ($rows as $row)
		{
			...
			if ($config->activate_deposit_feature)
			{
				...
				if ($forImport)
				{
					$row->deposit_payment_status = $row->payment_status;
				}
				else
				{
					...
					{
						case 0:
							$row->deposit_payment_status = Text::_('EB_PARTIAL_PAYMENT');
							break;
						case 1:
							$row->deposit_payment_status = Text::_('EB_FULL_PAYMENT');
							break;
						case 2:
							$row->deposit_payment_status = Text::_('EB_DEPOSIT_PAID');
							break;
						default:
			...
			if ($config->activate_tickets_pdf)
			{
				...
				{
					$row->ticket_number = EventbookingHelperTicket::formatTicketNumber(
						$row->ticket_prefix,
						$row->ticket_number,
						$config
					);
			...
			{
				$row->registration_cancel_date = HTMLHelper::_(
					'date',
	...
	public static function excelExport($fields, $rows, $filename, $headers = [])
	{
		...
		else
		{
			...
			if (is_dir(Path::clean($tmpPath)) && is_writable($tmpPath))
			{
