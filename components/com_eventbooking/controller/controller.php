...
class EventbookingController extends RADController
{
	...
	public function search()
	{
		...
		$filterCity     = $this->input->getString('filter_city');
		$filterDistance = $this->input->getInt('filter_distance', 0);
		$filterOrder    = $this->input->getString('filter_order', '');
		$filterOrderDir = $this->input->getString('filter_order_Dir', '');
	...
	public function upload_file()
	{
		...
		if (in_array(strtolower($fileExt), $allowedExtensions))
		{
			...
			if ($canUpload)
			{
				...
				{
					$json['error'] = Text::sprintf('EB_FILE_UPLOAD_FAILED', $fileName);
				}
			}
		}
		else
