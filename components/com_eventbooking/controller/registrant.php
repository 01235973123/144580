...
class EventbookingControllerRegistrant extends RADControllerAdmin
{
	...
	use EventbookingControllerView;
	use RADControllerDownload;
	use EventbookingControllerCommonRegistrant;
	use EventbookingControllerRegistrantExport;
	use EventbookingControllerRegistrantCheckin;
	...
	public function cancel()
	{
		...
		{
			$redirectUrl = Route::_(
				'index.php?option=com_eventbooking&Itemid=' . $config->get('default_menu_item_' . $language)
			);
		}
		...
		$query->select(
			'a.id, a.event_date, a.cancel_before_date, a.enable_cancel_registration, b.user_id, b.id AS registrant_id'
		)
	...
	public function download_certificate()
	{
		...
		if (empty($downloadCode) && $row->user_id != $user->id && StringHelper::strtolower(
				$row->email
			) != StringHelper::strtolower($user->email))
		{
			throw new Exception(Text::_('You do not have permission to download the certificate'), 403);
		}
	...
	public function refund()
	{
		...
		if (EventbookingHelperAcl::canManageRegistrant(
				$rowRegistrant
			) && EventbookingHelperRegistration::canRefundRegistrant($rowRegistrant))
		{
