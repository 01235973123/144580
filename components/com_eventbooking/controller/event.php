...
class EventbookingControllerEvent extends EventbookingController
{
	...
	public function save()
	{
		...
		if ($config->enable_captcha && !$this->validateCaptcha($this->input))
		{
		...
		if ($task == 'save2copy')
		{
			...
			$return = Route::_(
				'index.php?option=com_eventbooking&view=event&id=' . $this->input->getInt(
					'id',
					0
				) . '&layout=' . $formLayout . '&Itemid=' . $this->input->getInt('Itemid'),
				false
			);
		...
		{
			$this->setRedirect(
				Route::_(EventbookingHelperRoute::getViewRoute('events', $this->input->getInt('Itemid')), false),
				$msg
			);
		}
		else
	...
	public function publish()
	{
		...
		foreach ($cid as $id)
		{
			if (!EventbookingHelperAcl::canChangeEventStatus($id))
			{
				...
				$this->setRedirect(
					Route::_(EventbookingHelperRoute::getViewRoute('events', $this->input->getInt('Itemid', 0)), false),
					$msg
				);
	...
	public function send_invite()
	{
		...
		if ($config->show_invite_friend)
		{
			...
			$this->setRedirect(
				Route::_(
					'index.php?option=com_eventbooking&view=invite&layout=complete&tmpl=component&Itemid=' . $this->input->getInt(
						'Itemid',
						0
					),
					false
