...
use OSSolution\EventBooking\Admin\Event\Registration\ValidateFormData;
...
class EventbookingControllerRegister extends EventbookingController
{
	...
	public function check_event_password()
	{
		...
		if ($success)
		{
			$this->app->getSession()->set('eb_passowrd_' . $eventId, 1);
		...
		else
		{
			...
			$url    = Route::_(
				'index.php?option=com_eventbooking&view=password&event_id=' . $eventId . '&return=' . $return . '&Itemid=' . $Itemid,
				false
			);
	...
	public function individual_registration()
	{
		...
		if (!$eventId)
		{
			$this->app->enqueueMessage(Text::_('Invalid Event ID: 0'), 'warning');
		...
		if (!$event)
		{
			$this->app->enqueueMessage(Text::_('EB_EVENT_NOT_FOUND'), 'warning');
		...
		if ($params->get('simply_registration_process', $config->simply_registration_process)
			...
			&& $user->id)
		{
			if (!EventbookingHelper::callOverridableHelperMethod('Registration', 'acceptRegistration', [$event]))
			{
				...
				if (!$waitingList || !$registrationOpen)
				{
					...
					$app->redirect(
						Route::_(EventbookingHelperRoute::getEventRoute($event->id, $event->main_category_id), false)
					);
				}
			}
			...
			if ($return === 1)
			{
				...
				$this->setRedirect(
					Route::_(
						'index.php?option=com_eventbooking&view=complete&Itemid=' . $this->input->getInt('Itemid'),
						false
					)
				);
			}
			elseif ($return === 2)
			{
				...
				$this->setRedirect(
					Route::_(
						'index.php?option=com_eventbooking&view=waitinglist&Itemid=' . $this->input->getInt('Itemid'),
						false
					)
				);
			}
	...
	public function process_individual_registration()
	{
		...
		if (!$user->id && $config->user_registration)
		{
			$errors = array_merge(
				$errors,
				EventbookingHelperValidator::validateUsername($input->post->get('username', '', 'raw'))
			);
			$errors = array_merge(
				$errors,
				EventbookingHelperValidator::validatePassword($input->post->get('password1', '', 'raw'))
			);
		}
		...
		{
			$numberWaitingListAvailable = $event->waiting_list_capacity - EventbookingHelperRegistration::countNumberWaitingList(
					$event
				);
		...
		if ($couponCode = $input->getString('coupon_code'))
		{
			...
			if (!$coupon)
			{
				$errors[] = Text::sprintf('EB_THE_ENTERED_COUPON_CODE_IS_INVALID', $couponCode);
			}
		}
	...
	{
		$config = EventbookingHelper::getConfig();
		$this->app->getSession()->set('eb_number_registrants', $this->input->getInt('number_registrants'));
	...
	public function store_group_members_data()
	{
		...
		$this->app->getSession()->set('eb_group_members_data', serialize($membersData));
		$eventId         = $this->input->getInt('event_id', 0);
	...
	public function store_billing_data_and_display_group_members_form()
	{
		...
		$this->app->getSession()->set('eb_group_billing_data', serialize($billingData));
	...
	public function process_group_registration($bypassBilling = false)
	{
		...
		if ($collectMemberInformation && empty($membersData))
		{
		...
		if ($event->event_capacity)
		{
			...
			{
				$errors[] = Text::sprintf(
					'EB_NUMBER_REGISTRANTS_ERROR',
					$numberRegistrants,
					$numberRegistrantsAvailable
				);
			}
	...
	public function calculate_remainder_fee()
	{
		...
		$response['payment_processing_fee'] = EventbookingHelper::formatAmount(
	...
	public function calculate_individual_registration_fee()
	{
		...
		$response['late_fee']               = EventbookingHelper::formatAmount($fees['late_fee'], $config);
	...
	protected function validateFormData($eventId, $registrationType, $data)
	{
		...
		if ($registrationType == 0 && EventbookingHelperRegistration::isCollectMemberInformationForTicketTypes($event))
		{
			// Use group billing fields in case ticket types with collect member information
			$registrationTypeForGettingFormFields = 1;
		}
		else
		{
			// No change, use data from $registrationType field
			$registrationTypeForGettingFormFields = $registrationType;
		}

		$typeOfRegistration = EventbookingHelperRegistration::getTypeOfRegistration($event);
		$rowFields          = EventbookingHelperRegistration::getFormFields(
			$eventId,
			$registrationTypeForGettingFormFields,
			null,
			null,
			$typeOfRegistration
		);
		...
		if ($registrationType == 0 && PluginHelper::isEnabled('eventbooking', 'updatetotalregistrants'))
		{
			...
			foreach ($rowFields as $rowField)
			{
				if (str_starts_with($rowField->name, 'number_') && !empty($data[$rowField->name]))
				{
			...
			if ($event->event_capacity > 0)
			{
				...
				{
					$errors[] = Text::sprintf(
						'EB_NUMNER_REGISTRANTS_EXCEED_LIMIT',
						$numberRegistrantsAvailable,
						$totalRegistrants
					);
				}
		...
		$eventObj = new ValidateFormData(
			'onEBValidateFormData',
			['event' => $event, 'registrationType' => $registrationType, 'data' => $data]
		);

		$validateResults = $this->app->triggerEvent('onEBValidateFormData', $eventObj);
	...
	protected function validateTickets($event, $input, &$data, $waitingListEnabled)
	{
		...
		{
			if (isset($data['ticket_type_' . $ticketType->id]))
			{
			...
			if ($quantity > 0)
			{
				...
				$totalNumberTickets += $quantity * $weight;
				$numberSelectedTicketTypes++;
			}

			if ($ticketType->min_tickets_per_booking > 0 && $quantity < $ticketType->min_tickets_per_booking)
			{
				$errors[] = Text::sprintf(
					'EB_MINIMUM_QUANTITY_REQUIRED_WARNING',
					$ticketType->min_tickets_per_booking,
					$ticketType->title
				);
			}

			if ($quantity > 0 && $ticketType->capacity > 0)
			{
				// Validate quantity
				...
				{
					$errors[] = Text::sprintf(
						'EB_TICKET_QUANTITY_INVALID_WARNING',
						$quantity,
						$ticketType->title,
						$availableQuantity
					);
			...
			if ($quantity > 0 && $collectMembersInformation)
			{
				for ($i = 0; $i < $quantity; $i++)
				{
					...
					if (count($memberErrors))
					{
						...
						{
							$errors[] = Text::sprintf(
									'EB_MEMBER_TICKET_VALIDATION_ERROR',
									$ticketType->title,
									$i + 1
								) . ' ' . $memberError;
						}
		...
		{
			$errors[] = Text::sprintf(
				'EB_MAX_NUMBER_OF_TICKETS_CAN_PURCHASE',
				$event->event_capacity - $event->total_registrants
			);
		}
