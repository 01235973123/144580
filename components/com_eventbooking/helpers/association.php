...
class EventbookingHelperAssociation
{
	...
	public static function getAssociations()
	{
		...
		unset($languages[Factory::getApplication()->getLanguage()->getTag()]);

		/* @var DatabaseDriver $db */
		$db    = Factory::getContainer()->get('db');
		$query = $db->getQuery(true);

		if ($view === 'event' && !in_array($layout, ['form', 'simple']))
		{
			$query->select('*')
				->from('#__eb_events')
				->where('id = ' . (int) $id);
			...
			foreach ($languages as $tag => $language)
			{
				...
				if (empty($event->{'title' . $fieldSuffix}) || ($event->language != '*' && $event->language != $tag))
				{
					continue;
				}

				$result[$tag] = EventbookingHelperRoute::getEventRoute(
						$event->id,
		...
		if ($view === 'category')
		{
			...
			foreach ($languages as $tag => $language)
			{
				...
				$result[$tag] = EventbookingHelperRoute::getCategoryRoute(
						$category->id,
						$defaultItemId,
						$tag
					) . '&al=' . $tag;
