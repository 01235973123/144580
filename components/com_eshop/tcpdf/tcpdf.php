			...
			if ($dom[$key]['tag']) {
				if ($dom[$key]['opening']) {
					...
					if (($dom[$key]['value'] == 'td') OR ($dom[$key]['value'] == 'th')) {
						...
						// tuanpn: Fixed fatal error array_push(): Argument #1 ($array) must be of type array, null given
						$dom[$trid]['cellpos'] = $dom[$trid]['cellpos'] ?? [];
