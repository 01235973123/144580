...
{
	static function getPluginHelper(&$plugin, $params = null, $type, $name)
	{
		//$hash = md5('getPluginHelper_' . $plugin->get('_type') . '_' . $plugin->get('_name') . '_' . json_encode($params));
		$hash = md5('getPluginHelper_' . $type . '_' . $name . '_' . json_encode($params));
		...
		if (!$params)
		{
			...
			$params = nnParameters::getInstance()->getPluginParams($name);
		}

		//require_once JPATH_PLUGINS . '/' . $plugin->get('_type') . '/' . $plugin->get('_name') . '/helper.php';
		require_once JPATH_PLUGINS . '/' . $type . '/' . $name . '/helper.php';
