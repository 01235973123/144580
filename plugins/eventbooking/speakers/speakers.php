...
class plgEventBookingSpeakers extends CMSPlugin implements SubscriberInterface
{
	...
	private function drawSettingForm($row)
	{
		...
		$xml->field->attributes()->layout = $this->params->get(
			'subform_layout',
		...
		if ($row->id)
		{
			...
			foreach ($db->loadObjectList() as $speaker)
			{
				...
				{
					$speakerData[$customField] = $speaker->{$customField};
				}
	...
	public function onEventDisplay(Event $eventObj): void
	{
		...
		$result = [
			...
			'form'     => EventbookingHelperHtml::loadCommonLayout(
				'plugins/' . $layoutFile,
				['speakers' => $speakers, 'params' => $this->params]
			),
