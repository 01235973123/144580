...
class plgEventBookingMoveRegistrants extends CMSPlugin implements SubscriberInterface
{
	...
	public function onRegistrationCancelled(Event $eventObj): void
	{
		...
		if (!$this->params->get('allow_moving_after_event_started', 1)
	...
	/**
	 * Method to check if the event is already started
	 *
	 * @param   EventbookingTableEvent  $event
	 *
	 * @return bool
	 */
	private function isEventStarted($event): bool
	{
		$now       = Factory::getDate('now', $this->app->get('offset'));
		$eventDate = Factory::getDate($event->event_date, $this->app->get('offset'));
