...
define('ESHOP_RAD_INPUT_ALLOWRAW', 2);
define('ESHOP_RAD_INPUT_ALLOWHTML', 4);

use Joomla\CMS\Filter\InputFilter;
use Joomla\CMS\Input\Input;

/**
 * Extends Input class to allow getting raw data from Input object. This can be removed when we don't provide support for Joomla 2.5.x
 *
 * @package       EshopRAD
 * @subpackage    Input
 * @since         1.0
 *
 * @property-read    EshopRADInput $get
 * @property-read    EshopRADInput $post
 */
class EshopRADInput extends Input
{
	/**
	 * Constructor.
	 *
	 ...
	 * @return mixed
	 */
	public function getData($mask = ESHOP_RAD_INPUT_ALLOWHTML)
	{
		if ($mask & 2)
		{
			return $this->data;
	...
	public function __get($name)
	{
		...
		{
			$this->inputs[$name] = new EshopRADInput($GLOBALS[$superGlobal], $this->options);
