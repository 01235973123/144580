	...
	<div id="j-main-container">
		<div id="filter-bar" class="btn-toolbar<?php if ($isJoomla4) echo ' js-stools-container-filters-visible'; ?>">
			<div class="filter-search btn-group pull-left">
				<label for="filter_search" class="element-invisible"><?php echo Text::_('ESHOP_FILTER_SEARCH_CUSTOMERS_DESC');?></label>
				<input type="text" name="search" id="filter_search" placeholder="<?php echo Text::_('JSEARCH_FILTER'); ?>" value="<?php echo $this->escape($this->state->search); ?>" class="hasTooltip form-control" title="<?php echo HTMLHelper::tooltipText('ESHOP_SEARCH_CUSTOMERS_DESC'); ?>" />
			</div>
			...
			<div class="btn-group pull-right hidden-phone">
				<?php
				    echo $this->lists['customergroup_id'];
		...
		<table class="adminlist table table-striped" id="recordsList">
			<thead>
				<tr>
					...
					<th class="text_left" width="15%">
						<?php echo HTMLHelper::_('grid.sort',  Text::_('ESHOP_NAME'), 'b.name', $this->lists['order_Dir'], $this->lists['order'] ); ?>
					</th>
					<th class="text_left" width="15%">
						<?php echo Text::_('ESHOP_CUSTOMER_GROUP'); ?>
					</th>
					...
					<th class="text_left" width="15%">
						<?php echo HTMLHelper::_('grid.sort',  Text::_('ESHOP_TELEPHONE'), 'a.telephone', $this->lists['order_Dir'], $this->lists['order'] ); ?>
					</th>
					<th class="text_left" width="15%">
						<?php echo HTMLHelper::_('grid.sort',  Text::_('ESHOP_FAX'), 'a.fax', $this->lists['order_Dir'], $this->lists['order'] ); ?>
					</th>
			...
			{
				$row = &$this->items[$i];
				$link 	= Route::_( 'index.php?option=com_eshop&task=customer.edit&cid[]='. $row->id);
				$customerGroupLink = Route::_( 'index.php?option=com_eshop&task=customergroup.edit&cid[]='. $row->customergroup_id);
				$checked 	= HTMLHelper::_('grid.id',   $i, $row->id );				
				$published 	= HTMLHelper::_('jgrid.published', $row->published, $i, 'customer.');
				?>
					...
					if (!$isJoomla4)
					{
					    ...
					    <td class="text_center">
    						<div class="btn-group">
    							<?php
    							echo $published;
    							echo $this->addDropdownList(Text::_('ESHOP_DELETE'), 'trash', $i, 'customer.remove');
