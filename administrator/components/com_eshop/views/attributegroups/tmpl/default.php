...
if ($ordering)
{
    $saveOrderingUrl = 'index.php?option=com_eshop&task=attributegroup.save_order_ajax';
...
<form action="index.php?option=com_eshop&view=attributegroups" method="post" name="adminForm" id="adminForm">
	<div id="j-main-container">
		...
		<table class="adminlist table table-striped" id="recordsList">
			<thead>
				<tr>
					...
					<th class="text_left" width="40%">
						<?php echo HTMLHelper::_('searchtools.sort',  Text::_('ESHOP_NAME'), 'b.attributegroup_name', $this->lists['order_Dir'], $this->lists['order'] ); ?>				
					</th>
					<th class="text_center" width="10%">
                        <?php echo Text::_('ESHOP_NUMBER_ATTRIBUTES'); ?>
                    </th>
			...
			{
				$row = &$this->items[$i];
				$link 	= Route::_( 'index.php?option=com_eshop&task=attributegroup.edit&cid[]='. $row->id);
				$checked 	= HTMLHelper::_('grid.id',   $i, $row->id );
				$published 	= HTMLHelper::_('jgrid.published', $row->published, $i, 'attributegroup.');
				?>
				<tr class="<?php echo "row$k"; ?>" data-draggable-group="none">
					...
					if (!$isJoomla4)
					{
					    ...
					    <td class="text_center">
    						<div class="btn-group">
    							<?php
    							echo $published;
    							echo $this->addDropdownList(Text::_('ESHOP_COPY'), 'copy', $i, 'attributegroup.copy');
    							echo $this->addDropdownList(Text::_('ESHOP_DELETE'), 'trash', $i, 'attributegroup.remove');
					...
					<td class="text_center">
                        <a href="index.php?option=com_eshop&view=attributes&attributegroup_id=<?php echo $row->id; ?>"><?php echo EShopHelper::numberAttributes($row->id); ?></a>
                    </td>
					<td class="text_center">
						<?php echo $published; ?>
					</td>
