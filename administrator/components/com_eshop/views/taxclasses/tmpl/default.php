	...
	<div id="j-main-container">
		<div id="filter-bar" class="btn-toolbar<?php if ($isJoomla4) echo ' js-stools-container-filters-visible'; ?>">
			<div class="filter-search btn-group pull-left">
				<label for="filter_search" class="element-invisible"><?php echo Text::_('ESHOP_FILTER_SEARCH_TAXCLASSES_DESC');?></label>
				<input type="text" name="search" id="filter_search" placeholder="<?php echo Text::_('JSEARCH_FILTER'); ?>" value="<?php echo $this->escape($this->state->search); ?>" class="hasTooltip form-control" title="<?php echo HTMLHelper::tooltipText('ESHOP_SEARCH_TAXCLASSES_DESC'); ?>" />
			</div>
		...
		<table class="adminlist table table-striped">
			...
			<tr>
				...
				<th class="text_left" width="40%">
					<?php echo HTMLHelper::_('grid.sort',  Text::_('ESHOP_NAME'), 'a.taxclass_name', $this->lists['order_Dir'], $this->lists['order'] ); ?>
				</th>
				...
				<th class="text_center" width="15%">
					<?php echo HTMLHelper::_('grid.sort',  Text::_('ESHOP_MODIFIED_DATE'), 'a.modified_date', $this->lists['order_Dir'], $this->lists['order'] ); ?>
				</th>
			...
			{
				$row = &$this->items[$i];
				$link 	= Route::_( 'index.php?option=com_eshop&task=taxclass.edit&cid[]='. $row->id);
				$checked 	= HTMLHelper::_('grid.id',   $i, $row->id );
				$published 	= HTMLHelper::_('jgrid.published', $row->published, $i, 'taxclass.');
				?>
				<tr class="<?php echo "row$k"; ?>">
					...
					if (!$isJoomla4)
					{
					    ...
					    <td class="text_center">
    						<div class="btn-group">
    							<?php
    							echo $published;
    							echo $this->addDropdownList(Text::_('ESHOP_COPY'), 'copy', $i, 'taxclass.copy');
    							echo $this->addDropdownList(Text::_('ESHOP_DELETE'), 'trash', $i, 'taxclass.remove');
					...
					<td class="text_center">
						<?php echo HTMLHelper::_('date', $row->created_date, $dateFormat, null); ?>
					</td>
					<td class="text_center">
						<?php echo HTMLHelper::_('date', $row->modified_date, $dateFormat, null); ?>
					</td>
					<td class="text_center">
						<?php echo $published; ?>
					</td>
