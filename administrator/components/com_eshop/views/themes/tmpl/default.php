...
if ($ordering)
{
    $saveOrderingUrl = 'index.php?option=com_eshop&task=theme.save_order_ajax';
	...
	<div id="j-main-container">
		<div id="filter-bar" class="btn-toolbar<?php if ($isJoomla4) echo ' js-stools-container-filters-visible'; ?>">
			<div class="filter-search btn-group pull-left">
				<label for="filter_search" class="element-invisible"><?php echo Text::_('ESHOP_FILTER_SEARCH_THEMES_DESC');?></label>
				<input type="text" name="search" id="filter_search" placeholder="<?php echo Text::_('JSEARCH_FILTER'); ?>" value="<?php echo $this->escape($this->state->search); ?>" class="hasTooltip form-control" title="<?php echo HTMLHelper::tooltipText('ESHOP_SEARCH_THEMES_DESC'); ?>" />
			</div>
		...
		<table class="adminlist table table-striped" id="recordsList">
			...
			{
				$row = &$this->items[$i];
				$link 	= Route::_( 'index.php?option=com_eshop&task=theme.edit&cid[]='. $row->id );
				$checked 	= HTMLHelper::_('grid.id',   $i, $row->id );
				$published 	= HTMLHelper::_('jgrid.published', $row->published, $i, 'theme.');
				?>
					...
					if (!$isJoomla4)
					{
					    ...
					    <td class="text_center">
    						<div class="btn-group">
    							<?php
    							echo $published;
    							echo $this->addDropdownList(Text::_('ESHOP_DELETE'), 'trash', $i, 'theme.remove');
			...
			<tr>
				<td>
					<fieldset class="form-horizontal options-form">
						<legend><?php echo Text::_('ESHOP_INSTALL_NEW_THEME'); ?></legend>
						<table>
							<tr>
								<td>
									<input type="file" name="theme_package" id="theme_package" size="57" class="input_box" />
									<input type="button" class="btn btn-primary" value="<?php echo Text::_('ESHOP_INSTALL'); ?>" onclick="installTheme();" />
								</td>
							</tr>
						</table>					
					</fieldset>
	...
	<script type="text/javascript">
		function installTheme() {
			...
			if (form.theme_package.value =="") {
				alert("<?php echo Text::_('ESHOP_CHOOSE_THEME_PLUGIN'); ?>");
				return;	
			}
