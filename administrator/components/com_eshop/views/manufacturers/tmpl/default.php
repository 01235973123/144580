...
if ($ordering)
{
    $saveOrderingUrl = 'index.php?option=com_eshop&task=manufacturer.save_order_ajax';
	...
	<div id="j-main-container">
		<div id="filter-bar" class="btn-toolbar<?php if ($isJoomla4) echo ' js-stools-container-filters-visible'; ?>">
			<div class="filter-search btn-group pull-left">
				<label for="filter_search" class="element-invisible"><?php echo Text::_('ESHOP_FILTER_SEARCH_MANUFACTURERS_DESC');?></label>
				<input type="text" name="search" id="filter_search" placeholder="<?php echo Text::_('JSEARCH_FILTER'); ?>" value="<?php echo $this->escape($this->state->search); ?>" class="hasTooltip form-control" title="<?php echo HTMLHelper::tooltipText('ESHOP_SEARCH_MANUFACTURERS_DESC'); ?>" />
			</div>
		...
		<table class="adminlist table table-striped" id="recordsList">
			...
			<tr>
				...
				<th class="text_left" width="40%">
					<?php echo HTMLHelper::_('searchtools.sort',  Text::_('ESHOP_NAME'), 'b.manufacturer_name', $this->lists['order_Dir'], $this->lists['order'] ); ?>
				</th>
				...
				<th width="5%" class="text_center">
					<?php echo HTMLHelper::_('searchtools.sort',  Text::_('ESHOP_HITS'), 'a.hits', $this->lists['order_Dir'], $this->lists['order'] ); ?>
				</th>
				<th width="5%" class="text_center">
			...
			{
				$row = &$this->items[$i];
				$link 	= Route::_( 'index.php?option=com_eshop&task=manufacturer.edit&cid[]='. $row->id);
				$checked 	= HTMLHelper::_('grid.id',   $i, $row->id );
				$published 	= HTMLHelper::_('jgrid.published', $row->published, $i, 'manufacturer.');
				?>
				<tr class="<?php echo "row$k"; ?>" data-draggable-group="none">
					...
					if (!$isJoomla4)
					{
					    ...
					    <td class="text_center">
    						<div class="btn-group">
    							<?php
    							echo $published;
    							echo $this->addDropdownList(Text::_('ESHOP_COPY'), 'copy', $i, 'manufacturer.copy');
    							echo $this->addDropdownList(Text::_('ESHOP_DELETE'), 'trash', $i, 'manufacturer.remove');
					...
					<td>
						...
						<p class="smallsub">
							<?php echo Text::sprintf('JGLOBAL_LIST_ALIAS', $this->escape($row->manufacturer_alias)); ?></p>
					</td>
					<td class="text_center">
						<?php
						if (is_file(JPATH_ROOT.'/media/com_eshop/manufacturers/'.$row->manufacturer_image))
						{
							$viewImage = File::stripExt($row->manufacturer_image).'-100x100.'.EShopHelper::getFileExt($row->manufacturer_image);

							if (is_file(JPATH_ROOT.'/media/com_eshop/manufacturers/resized/'.$viewImage))
							{
							?>
								<img src="<?php echo $rootUri . 'media/com_eshop/manufacturers/resized/' . $viewImage; ?>" width="50" />
							<?php
							}
							else
							{
							?>
								<img src="<?php echo $rootUri . 'media/com_eshop/manufacturers/' . $row->manufacturer_image; ?>" width="50" />
							<?php
							}
						}
						?>
