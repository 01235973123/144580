	...
	<div id="j-main-container">
		<div id="filter-bar" class="btn-toolbar<?php if ($isJoomla4) echo ' js-stools-container-filters-visible'; ?>">
			<div class="filter-search btn-group pull-left">
				<label for="filter_search" class="element-invisible"><?php echo Text::_('ESHOP_FILTER_SEARCH_ATTRIBUTES_DESC');?></label>
				<input type="text" name="search" id="filter_search" placeholder="<?php echo Text::_('JSEARCH_FILTER'); ?>" value="<?php echo $this->escape($this->state->search); ?>" class="hasTooltip form-control" title="<?php echo HTMLHelper::tooltipText('ESHOP_SEARCH_ATTRIBUTES_DESC'); ?>" />
			</div>
		...
		<table class="adminlist table table-striped" id="recordsList">
			...
			<tr>
				...
				<th class="text_left" width="30%">
					<?php echo HTMLHelper::_('searchtools.sort',  Text::_('ESHOP_NAME'), 'b.attribute_name', $this->lists['order_Dir'], $this->lists['order'] ); ?>
				</th>
				<th class="text_left" width="30%">
			...
			{
				$row = &$this->items[$i];
				$link 	= Route::_( 'index.php?option=com_eshop&task=attribute.edit&cid[]='. $row->id);
				$checked 	= HTMLHelper::_('grid.id',   $i, $row->id );
				$published 	= HTMLHelper::_('jgrid.published', $row->published, $i, 'attribute.');
				?>
				<tr class="<?php echo "row$k"; ?>" data-draggable-group="none">
					...
					if (!$isJoomla4)
					{
					    ...
					    <td class="text_center">
    						<div class="btn-group">
    							<?php
    							echo $published;
    							echo $this->addDropdownList(Text::_('ESHOP_COPY'), 'copy', $i, 'attribute.copy');
    							echo $this->addDropdownList(Text::_('ESHOP_DELETE'), 'trash', $i, 'attribute.remove');
					...
					<td>
						...
						?>
						<a href="<?php echo Route::_( 'index.php?option=com_eshop&task=attributegroup.edit&cid[]='. $attributeGroup->attributegroup_id); ?>">
