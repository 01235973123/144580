...
if ($ordering)
{
	$saveOrderingUrl = 'index.php?option=com_eshop&task=category.save_order_ajax';
...
if (count($this->items))
{
	...
	{
		$this->ordering[$item->category_parent_id][] = $item->id;
	}
}
?>
	...
	<div id="j-main-container">
		<div id="filter-bar" class="btn-toolbar<?php if ($isJoomla4) echo ' js-stools-container-filters-visible'; ?>">
			<div class="filter-search btn-group pull-left">
				<label for="filter_search" class="element-invisible"><?php echo Text::_('ESHOP_FILTER_SEARCH_CATEGORIES_DESC');?></label>
				<input type="text" name="search" id="filter_search" placeholder="<?php echo Text::_('JSEARCH_FILTER'); ?>" value="<?php echo $this->escape($this->state->search); ?>" class="hasTooltip form-control" title="<?php echo HTMLHelper::tooltipText('ESHOP_SEARCH_CATEGORIES_DESC'); ?>" />
			</div>
		...
		<table class="adminlist table table-striped" id="recordsList">
			<thead>
				<tr>
					...
					<th class="text_left" width="40%">
						<?php echo HTMLHelper::_('searchtools.sort',  Text::_('ESHOP_NAME'), 'b.category_name', $this->lists['order_Dir'], $this->lists['order'] ); ?>				
					</th>
					<th class="text_center" width="15%">
    					<?php echo Text::_('ESHOP_IMAGE'); ?>
    				</th>
					<th class="text_center" width="10%">
                        <?php echo Text::_('ESHOP_NUMBER_PRODUCTS'); ?>
                    </th>
			...
			{
				$row = &$this->items[$i];
				$link 	= Route::_( 'index.php?option=com_eshop&task=category.edit&cid[]='. $row->id);
				...
				{
					$parentsStr = "";
					$_currentParentId = $row->category_parent_id;
				...
				{
				    ?>
                    <tr class="row<?php echo $i % 2; ?>" data-draggable-group="<?php echo $row->category_parent_id; ?>" item-id="<?php echo $row->id ?>" parents="<?php echo $parentsStr ?>" level="<?php echo $row->level ?>">
                <?php
                }
				else
                {
                ?>
                    <tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $row->category_parent_id; ?>" item-id="<?php echo $row->id ?>" parents="<?php echo $parentsStr ?>" level="<?php echo $row->level ?>">
                <?php
                }
				?>
					...
					if (!$isJoomla4)
					{
					    ...
					    <td class="text_center">
    						<div class="btn-group">
    							<?php
    							echo HTMLHelper::_('jgrid.published', $row->published, $i, 'category.');
    							echo $this->addDropdownList(Text::_('ESHOP_COPY'), 'copy', $i, 'category.copy');
    							echo $this->addDropdownList(Text::_('ESHOP_DELETE'), 'trash', $i, 'category.remove');
					...
					<td class="text_center">
						<?php
						if (is_file(JPATH_ROOT.'/media/com_eshop/categories/'.$row->category_image))
						{
							$viewImage = File::stripExt($row->category_image).'-100x100.'.EShopHelper::getFileExt($row->category_image);

							if (is_file(JPATH_ROOT.'/media/com_eshop/categories/resized/'.$viewImage))
							{
							?>
								<img src="<?php echo $rootUri . 'media/com_eshop/categories/resized/' . $viewImage; ?>" width="50" />
							<?php
							}
							else
							{
							?>
								<img src="<?php echo $rootUri . 'media/com_eshop/categories/' . $row->category_image; ?>" width="50" />
							<?php
							}
						}
						?>
					...
					<td class="text_center">
						<a href="index.php?option=com_eshop&view=products&category_id=<?php echo $row->id; ?>"><?php echo $row->number_products; ?></a>
					</td>
					<td class="text_center">
						<?php echo $published; ?>
					</td>
