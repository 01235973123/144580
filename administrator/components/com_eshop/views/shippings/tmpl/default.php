	...
	<div id="j-main-container">
		<div id="filter-bar" class="btn-toolbar<?php if ($isJoomla4) echo ' js-stools-container-filters-visible'; ?>">
			<div class="filter-search btn-group pull-left">
				<label for="filter_search" class="element-invisible"><?php echo Text::_('ESHOP_FILTER_SEARCH_SHIPPINGS_DESC');?></label>
				<input type="text" name="search" id="filter_search" placeholder="<?php echo Text::_('JSEARCH_FILTER'); ?>" value="<?php echo $this->escape($this->state->search); ?>" class="hasTooltip form-control" title="<?php echo HTMLHelper::tooltipText('ESHOP_SEARCH_SHIPPINGS_DESC'); ?>" />
			</div>
		...
		<table class="adminlist table table-striped" id="recordsList">
			...
			{
				$row = &$this->items[$i];
				$link 	= Route::_( 'index.php?option=com_eshop&task=shipping.edit&cid[]='. $row->id );
				$checked 	= HTMLHelper::_('grid.id',   $i, $row->id );
				$published 	= HTMLHelper::_('jgrid.published', $row->published, $i, 'shipping.');
				?>
					...
					if (!$isJoomla4)
					{
					    ...
					    <td class="text_center">
    						<div class="btn-group">
    							<?php
    							echo $published;
    							echo $this->addDropdownList(Text::_('ESHOP_DELETE'), 'trash', $i, 'shipping.remove');
			...
			<tr>
				<td>
					<fieldset class="form-horizontal options-form">
						<legend><?php echo Text::_('ESHOP_INSTALL_NEW_SHIPPING_PLUGIN'); ?></legend>
						<table>
							<tr>
								<td>
	...
	<script type="text/javascript">
		function installPlugin() {
			...
			if (form.plugin_package.value =="") {
				alert("<?php echo Text::_('ESHOP_CHOOSE_SHIPPING_PLUGIN'); ?>");
				return;	
			}
