...
class EventbookingControllerUpgrade370 extends RADController
{
	public function upgrade()
	{
		...
		EventbookingHelper::executeSqlFile($menuSql);

		$customAdminMenuSql = JPATH_ADMINISTRATOR . '/components/com_eventbooking/sql/custommenus.eventbooking.sql';

		if (file_exists($customAdminMenuSql))
		{
			EventbookingHelper::executeSqlFile($customAdminMenuSql);
		}
		...
		if (!in_array('alias', $fields))
		{
			...
			foreach ($rowLocations as $rowLocation)
			{
				...
				$sql = 'UPDATE #__eb_locations SET alias = ' . $db->quote(
						$locationAlias
					) . ' WHERE id = ' . $rowLocation->id;
				$db->setQuery($sql);
				$db->execute();
			}
		}
		...
		if (!in_array('max_end_date', $fields))
		{
			...
			foreach ($parentIds as $parentId)
			{
				...
				$sql = 'UPDATE #__eb_events SET max_end_date = ' . $db->quote(
						$maxEventDate
					) . ' WHERE id = ' . $parentId;
				$db->setQuery($sql);
				$db->execute();
		...
		if (!in_array('attachment', $fields))
		{
			...
			if (!is_dir(Path::clean(JPATH_ROOT . '/media/com_eventbooking')))
			{
		...
		if (!in_array('alias', $fields))
		{
			...
			if (count($rowEvents))
			{
				foreach ($rowEvents as $rowEvent)
				{
					...
					{
						$alias = ApplicationHelper::stringURLSafe($rowEvent->title);
					}
		...
		if (empty($config->certificate_layout))
		{
			...
			{
				$sql = 'UPDATE #__eb_configs SET config_value = ' . $db->quote(
						$invoiceFormat
					) . ' WHERE config_key="certificate_layout"';
			}
			else
			{
				$sql = 'INSERT INTO #__eb_configs(config_key, config_value) VALUES ("certificate_layout", ' . $db->quote(
						$invoiceFormat
					) . ')';
			}
		...
		if (!in_array('alias', $fields))
		{
			...
			if (count($rowCategories))
			{
				...
				{
					$alias = ApplicationHelper::stringURLSafe($rowCategory->name);
					$sql   = 'UPDATE #__eb_categories SET `alias`=' . $db->quote(
							$alias
						) . ' WHERE id=' . $rowCategory->id;
					$db->setQuery($sql);
					$db->execute();
				}
			}
		...
		if (!in_array('is_group_billing', $fields))
		{
			...
			if (count($groupIds))
			{
				$sql = 'UPDATE #__eb_registrants SET is_group_billing=1 WHERE id IN (' . implode(
						',',
						$groupIds
					) . ') OR number_registrants > 1';
				$db->setQuery($sql);
				$db->execute();
		...
		if (empty($config->invoice_format))
		{
			...
			if (property_exists($config, 'invoice_format'))
			{
				$sql = 'UPDATE #__eb_configs SET config_value = ' . $db->quote(
						$invoiceFormat
					) . ' WHERE config_key="invoice_format"';
			}
			else
			{
				$sql = 'INSERT INTO #__eb_configs(config_key, config_value) VALUES ("invoice_format", ' . $db->quote(
						$invoiceFormat
					) . ')';
			}
		...
		if (empty($config->invoice_format_cart))
		{
			...
			if (property_exists($config, 'invoice_format_cart'))
			{
				$sql = 'UPDATE #__eb_configs SET config_value = ' . $db->quote(
						$invoiceFormat
					) . ' WHERE config_key="invoice_format_cart"';
			}
			else
			{
				$sql = 'INSERT INTO #__eb_configs(config_key, config_value) VALUES ("invoice_format_cart", ' . $db->quote(
						$invoiceFormat
					) . ')';
			}
		...
		if (!in_array('is_core', $fields))
		{
			...
			if (count($rowFieldValues))
			{
				foreach ($rowFieldValues as $rowFieldValue)
				{
					...
					$sql        = 'UPDATE #__eb_field_values SET field_value=' . $db->quote(
							$fieldValue
						) . ' WHERE id=' . $rowFieldValue->id;
					$db->setQuery($sql);
					$db->execute();
				}
			}
			...
			foreach ($groupMemberFields as $fieldName => $showed)
			{
				...
				$sql = "UPDATE #__eb_fields SET display_in=" . $db->quote($displayIn) . ' WHERE name=' . $db->quote(
						$fieldName
					);
				$db->setQuery($sql);
				$db->execute();
		...
		$query->update('#__menu')
			->set(
				$db->quoteName('link') . '=' . $db->quote('index.php?option=com_eventbooking&view=location&layout=form')
			)
		...
		if (is_file(Path::clean(JPATH_ADMINISTRATOR . '/manifests/packages/pkg_eventbooking.xml')))
		{
			// Insert update site
		...
		if ($asset)
		{
			$rules        = $asset->rules;
			$rules        = str_replace(
				'eventbooking.registrants_management',
				'eventbooking.registrantsmanagement',
				$rules
			);
			$rules        = str_replace(
				'eventbooking.view_registrants_list',
				'eventbooking.viewregistrantslist',
				$rules
			);
			$asset->rules = $rules;
			$asset->store();
		...
		if ($moveEventsImages)
		{
			$this->app->redirect('index.php?option=com_eventbooking&task=upgrade370.migrate_event_images');
		}
		else
		{
			echo 'Done';
	...
	public function migrate_event_images()
	{
		...
		if (!is_dir(Path::clean(JPATH_ROOT . '/images/com_eventbooking')))
		{
