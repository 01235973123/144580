...
class EventbookingControllerUpgrade3176 extends RADController
{
	public function upgrade()
	{
		...
		if (empty($config->certificate_layout))
		{
			...
			$invoiceFormat = file_get_contents(
				JPATH_ADMINISTRATOR . '/components/com_eventbooking/sql/certificate_layout.html'
			);
		...
		if (empty($config->invoice_format))
		{
			//Need to insert default data into the system
			$invoiceFormat = file_get_contents(
				JPATH_ADMINISTRATOR . '/components/com_eventbooking/sql/invoice_format.html'
			);
		...
		{
			$invoiceFormat = file_get_contents(
				JPATH_ADMINISTRATOR . '/components/com_eventbooking/sql/invoice_format_cart.html'
			);
		...
		if ($moveEventsImages)
		{
			$app->redirect('index.php?option=com_eventbooking&task=update3176.migrate_event_images');
		}
	}
