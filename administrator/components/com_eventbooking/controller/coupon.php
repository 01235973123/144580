...
class EventbookingControllerCoupon extends EventbookingController
{
	...
	public function import()
	{
		...
		$fileExt   = strtolower(EventbookingHelper::getFileExt($fileName));

		if (!in_array($fileExt, ['csv', 'xlsx']))
		{
			$this->setRedirect(
				'index.php?option=com_eventbooking&view=coupon&layout=import',
		...
		try
		{
			...
			$this->setRedirect(
				...
				Text::sprintf('EB_NUMBER_COUPONS_IMPORTED', $numberImportedCoupons)
			);
		}
		catch (Exception $e)
		{
	...
	public function export()
	{
		...
		$results = $this->app->triggerEvent(
			...
			[$rows, &$fields, &$headers, 'coupons_list.xlsx']
		);
	...
	public function batch()
	{
		...
		$this->setRedirect(
			'index.php?option=com_eventbooking&view=coupons',
			Text::_('EB_COUPONS_SUCCESSFULLY_GENERATED')
		);
	}
}
