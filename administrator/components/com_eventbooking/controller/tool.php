...
class EventbookingControllerTool extends RADController
{
	...
	/***
	 * Generate missing invoice number
	 *
	 * @return void
	 * @throws Exception
	 */
	public function generate_missing_invoice_number()
	{
		...
		foreach ($rows as $row)
		{
			...
			{
				$row->invoice_number           = EventbookingHelperRegistration::getInvoiceNumber($row);
		...
		echo 'Successfully Generating Invoice Number For registrants missing invoice';
	}
			...
			foreach ($languages as $language)
			{
				...
				if (file_exists(JPATH_ROOT . '/language/' . $tag . '/' . $tag . '.com_eventbooking.ini'))
				{
					$mailer->addAttachment(
						JPATH_ROOT . '/language/' . $tag . '/' . $tag . '.com_eventbooking.ini',
						$tag . '.com_eventbooking.ini'
					);
				}
	...
	public function resize_large_images()
	{
		...
		if (count($images) == 0)
		{
			$this->setRedirect(
				...
				'Large Images were successfully resized for your events'
			);
		}
		else
		{
	...
	public function resize_thumb_images()
	{
		...
		if (count($images) == 0)
		{
			$this->setRedirect(
				'index.php?option=com_eventbooking&view=events',
				'Thumb Images were successfully resized for your events'
			);
		}
		else
		{
			foreach ($images as $imageObj)
			{
				...
				{
					if (!is_dir(JPATH_ROOT . '/media/com_eventbooking/images/thumbs/' . $imageObj->username))
					{
		...
		if (count($languages))
		{
			...
			foreach ($languages as $language)
			{
				...
				foreach ($fields as $field)
				{
					...
					catch (Exception $e)
					{
						$this->app->enqueueMessage(
							sprintf('Field %s already exist in table %s', $fieldName, '#__eb_categories')
						);
					}
				}
				...
				foreach ($fields as $field)
				{
					...
					{
						$this->app->enqueueMessage(
							sprintf('Field %s already exist in table %s', $fieldName, '#__eb_events')
						);
					}
				}
				...
				foreach ($fields as $field)
				{
					...
					{
						$this->app->enqueueMessage(
							sprintf('Field %s already exist in table %s', $fieldName, '#__eb_locations')
						);
					}
				}
			}
	...
	public function create_controller_override()
	{
		...
		{
			$phpCode = '<?php';
			File::write($controllerFile, $phpCode);
		}
	...
	public function update_countries_states_database()
	{
		...
		EventbookingHelper::executeSqlFile(
			JPATH_ADMINISTRATOR . '/components/com_eventbooking/sql/countries_states.sql'
		);
	...
	public function build_eu_tax_rules()
	{
		...
		$this->setRedirect(
			'index.php?option=com_eventbooking&view=taxes',
	...
	public function create_paymongo_webhook(): void
	{
		...
		{
			$this->app->enqueueMessage(
				'Please enter API Key for the payment plugin before the system can create webhook',
				'warning'
			);
		...
		$request = [
			'data' => [
				'attributes' => [
					'url'    => Uri::root(
						) . 'index.php?option=com_eventbooking&task=payment_confirm&payment_method=' . $name,
	...
	/**
	 * Method to fix missing AUTO_INCREMENT attribute for id field
	 * @return void
	 */
	public function fix_missing_auto_increment_for_id_column(): void
	{
		...
		foreach ($tables as $table)
		{
			if (!str_starts_with($table, $prefix . 'eb_'))
			{
				continue;
			}
			...
			if ($fields['id']->Extra === 'auto_increment')
			{
				continue;
			}

			// First, make sure id field is primary key of the table
			$indexes           = $db->getTableKeys($table);
			$hasPrimaryKeyOnId = false;
			...
			foreach ($indexes as $index)
			{
				...
				{
					$hasPrimaryKeyOnId = true;
					break;
				}
			}

			if (!$hasPrimaryKeyOnId)
			{
			...
			$sql = "ALTER TABLE $table CHANGE `id` `id` int unsigned NOT NULL AUTO_INCREMENT;";
			$db->setQuery($sql)
				->execute();
		}
	}
