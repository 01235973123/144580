...
class com_eventbookingInstallerScript
{
	...
	public function preflight($type, $parent)
	{
		...
		if (is_file(JPATH_ROOT . '/components/com_eventbooking/fields.xml'))
		{
	...
	private function restoreFiles()
	{
		...
		if (is_file(JPATH_ROOT . '/components/com_eventbooking/bak.fields.xml'))
		{
	...
	private function createFilesFolders()
	{
		...
		if (is_file(JPATH_ADMINISTRATOR . '/components/com_eventbooking/assets/css/custom.css'))
		{
			File::move(
	...
	public static function synchronizeDBSchema($installedVersion = '2.0.0')
	{
		...
		if (version_compare($installedVersion, '5.0.4', '<='))
		{
			...
			// Need to fix data for registration_type and language
			$query->clear()
				->update('#__eb_fields')
				->set('show_on_registration_type = 0')
				->where('show_on_registration_type IS NULL');
			$db->setQuery($query)
				->execute();
		...
		{
			$sql = "ALTER TABLE  `#__eb_speakers` ADD `published` tinyint NOT NULL DEFAULT 1;";
			$db->setQuery($sql)
				->execute();
		}
		...
		if (!in_array('published', $fields))
		{
			$sql = "ALTER TABLE  `#__eb_sponsors` ADD `published` tinyint NOT NULL DEFAULT 1;";
			$db->setQuery($sql)
				->execute();
		}
	}
	...
	private function migrateCommonLanguageItems()
	{
		...
		if (!is_file(JPATH_ADMINISTRATOR . '/language/en-GB/en-GB.com_eventbookingcommon.ini')
			|| !is_file(JPATH_ROOT . '/language/en-GB/en-GB.com_eventbooking.ini'))
		{
			return;
		}
