...
class EventbookingViewDiscountHtml extends RADViewItem
{
	...
	protected function prepareView()
	{
		...
		if ($config->hide_past_events_from_events_dropdown)
		{
			...
			{
				$query->where(
					'(id IN(' . $this->item->event_ids . ') OR DATE(event_date) >= ' . $currentDate . ' OR DATE(event_end_date) >= ' . $currentDate . ')'
				);
			}
			else
			{
				$query->where(
					'(DATE(event_date) >= ' . $currentDate . ' OR DATE(event_end_date) >= ' . $currentDate . ')'
				);
			}
		}
		...
		$this->lists['event_id'] = EventbookingHelperHtml::getEventsDropdown(
			$rows,
			'event_id[]',
			'class="input-xlarge form-select advSelect" multiple ',
			$selectedEventIds
		);
