...
<fieldset class="form-horizontal options-form eb-event-recurring-settings">
	...
	<div class="control-group" data-showon='<?php echo EventbookingHelperHtml::renderShowOn($showOnData); ?>'>
		...
		<div class="controls">
			<input type="number" name="recurring_frequency" id="recurring_frequency" class="form-control" value="<?php echo $this->item->recurring_frequency; ?>"/>
		</div>
	</div>
	...
	<div class="control-group" data-showon='<?php echo EventbookingHelperHtml::renderShowOn(['recurring_type' => '3']); ?>'>
		...
		<div class="controls">
			<input type="text" name="monthdays" id="monthdays"
				   class="form-control" value="<?php echo $this->item->monthdays; ?>" /> days in month (For Example 10,15,19)
		</div>
	</div>
	<div class="control-group" data-showon='<?php echo EventbookingHelperHtml::renderShowOn(['recurring_type' => '4']); ?>'>
		...
		<div class="controls">
			...
			echo HTMLHelper::_('select.genericlist', $options, 'week_in_month', 'class="form-select input-medium d-inline-block"', 'value', 'text', $params->get('week_in_month', 'first'));
			echo HTMLHelper::_('select.genericlist', $daysOfWeek, 'day_of_week', 'class="form-select input-medium d-inline-block"', 'value', 'text', $params->get('day_of_week', 'Sun'));
			?>
			of the month
		</div>
	</div>
	<div class="control-group" data-showon='<?php echo EventbookingHelperHtml::renderShowOn($showOnData); ?>'>
		...
		<div class="controls">
			<?php echo HTMLHelper::_('calendar', $this->item->recurring_end_date, 'recurring_end_date', 'recurring_end_date', $this->datePickerFormat, ['class' => 'input-small form-control']); ?>
		</div>
	</div>
	<div class="control-group" data-showon='<?php echo EventbookingHelperHtml::renderShowOn($showOnData); ?>'>
		<div class="control-label">
		...
		<div class="controls">
			<input type="number" name="recurring_occurrencies" size="5" class="form-control" value="<?php echo $this->item->recurring_occurrencies; ?>" />
		</div>
	</div>
	<?php
	if ($this->item->id)
