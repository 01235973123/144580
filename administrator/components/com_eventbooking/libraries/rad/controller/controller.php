...
class RADController
{
	...
	public function __construct(RADInput $input = null, array $config = [])
	{
		...
		$this->registerTask('__default', $config['default_task'] ?? 'display');
	}
	...
	public function display($cachable = false, array $urlparams = [])
	{
		...
		if ($view instanceof RADViewHtml && $this->app->isClient('site'))
		{
	...
	public function getView($name, $type = 'html', $layout = 'default', array $config = [])
	{
		...
		if ($this->app->isClient('site') && !is_dir(JPATH_ROOT . '/components/com_eventbooking/view/' . $name))
		{
	 ...
	 *
	 * @return \Joomla\CMS\Application\CMSApplication The application object.
	 */
