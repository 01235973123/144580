...
class RADModelAdmin extends RADModel
{
	...
	/**
	 * The event class for after delete event trigger
	 *
	 * @var string
	 */
	protected $eventAfterDeleteClass = RADEventModelAfterdelete::class;

	/**
	 * The event class for after save event trigger
	 *
	 * @var string
	 */
	protected $eventAfterSaveClass = RADEventModelAftersave::class;

	/**
	 * The event class for after before delete event trigger
	 *
	 * @var string
	 */
	protected $eventBeforeDeleteClass = RADEventModelBeforedelete::class;

	/**
	 * The event class for before save event trigger
	 *
	 * @var string
	 */
	protected $eventBeforeSaveClass = RADEventModelBeforesave::class;

	/**
	 * The event class for change state event trigger
	 *
	 * @var string
	 */
	protected $eventChangeStateClass = RADEventModelChangestate::class;
	...
	public function store($input, $ignore = [])
	{
		...
		elseif ($input->getInt('source_id'))
		{
			...
			// Reset created_date
			if (property_exists($row, 'created_date' && !$input->exists('created_date')))
			{
				$row->created_date = '';
			}
		}
		...
		if ($this->triggerEvents)
		{
			$beforeSaveEvent = new $this->eventBeforeSaveClass(
				$this->eventBeforeSave,
				['row' => $row, 'data' => $data, 'isNew' => $isNew]
			);

			$result = $app->triggerEvent($this->eventBeforeSave, $beforeSaveEvent);
		...
		if ($this->triggerEvents)
		{
			$afterSaveEvent = new $this->eventAfterSaveClass(
				$this->eventAfterSave,
				['row' => $row, 'data' => $data, 'isNew' => $isNew]
			);

			$app->triggerEvent($this->eventAfterSave, $afterSaveEvent);
		}
	...
	public function delete($cid = [])
	{
		...
		foreach ($cid as $id)
		{
			if ($row->load($id))
			{
				if ($this->triggerEvents)
				{
					$beforeDeleteEvent = new $this->eventBeforeDeleteClass(
						$this->eventBeforeDelete,
						['context' => $this->context, 'row' => $row]
					);

					$result = $app->triggerEvent($this->eventBeforeDelete, $beforeDeleteEvent);
				...
				if ($this->triggerEvents)
				{
					$afterDeleteEvent = new $this->eventAfterDeleteClass(
						$this->eventAfterDelete,
						['context' => $this->context, 'row' => $row]
					);

					$app->triggerEvent($this->eventAfterDelete, $afterDeleteEvent);
				}
			}
			else
			{
	...
	public function publish($pks, $value = 1)
	{
		...
		if ($this->triggerEvents)
		{
			$changeStateEvent = new $this->eventChangeStateClass($this->eventChangeState, [
				'context' => $this->context,
				'pks'     => $pks,
				'value'   => $value,
			]);

			Factory::getApplication()->triggerEvent($this->eventChangeState, $changeStateEvent);
		}
	...
	protected function initData()
	{
		...
		// For new record, id should be set to 0
			...
			if (Multilanguage::isEnabled())
			{
				...
				if (count($languages))
				{
					foreach ($languages as $language)
					{
						...
						if (!$row->{'alias_' . $sef})
						{
							$row->{'alias_' . $sef} = ApplicationHelper::stringURLSafe(
								$row->{$titleField . '_' . $sef}
							);
						}
						else
						{
