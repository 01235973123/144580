...
class RADFormFieldList extends RADFormField
{
	...
	protected function getOptions()
	{
		...
		elseif (is_string($this->row->values) && str_contains($this->row->values, "\r\n"))
		{
			$values = explode("\r\n", $this->row->values);
		}
		else
		...
		if ($this->row->quantity_field
			&& count($values)
			&& count($quantityValues)
			&& $this->eventId
			&& !$user->authorise('eventbooking.registrantsmanagement', 'com_eventbooking'))
		{
			...
			$values = EventbookingHelperHtml::getAvailableQuantityOptions(
				$values,
				$quantityValues,
				$this->eventId,
				$this->row->id,
		...
		if (count($values) == 0)
		{
			Factory::getApplication()->enqueueMessage(
				'There is no available option left for the field ' . $this->title,
				'warning'
			);
		...
		foreach ($values as $value)
		{
			...
			{
				$optionText = $optionValue . ' ' . Text::sprintf(
						'EB_QUANTITY_OPTION_AVAILABLE',
						$quantityValues[$optionValue]
					);
			}
