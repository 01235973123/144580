...
class RADForm
{
	...
	public function __construct($fields, $config = [])
	{
		...
		if ($hasInputMask)
		{
			// Add a check in case the form is initialized in very early state, where document is not available for application yet
			...
			if ($document)
			{
				$document->getWebAssetManager()
					->registerAndUseScript(
						'com_eventbooking.imask',
						'media/com_eventbooking/assets/js/imask/imask.min.js'
					);
			}
		}
	}
	...
	public function setValidatorLanguage($lang)
	{
		...
		if (file_exists(
			JPATH_ADMINISTRATOR . '/components/com_eventbooking/libraries/vendor/valitron/lang/' . $lang . '.php'
		))
		{
			$this->lang = $lang;
		...
		if (file_exists(
			JPATH_ADMINISTRATOR . '/components/com_eventbooking/libraries/vendor/valitron/lang/' . $parts[0] . '.php'
		))
		{
			$this->lang = $parts[0];
	...
	public function validate($prefix = '')
	{
		...
		foreach ($fields as $field)
		{
			...
			if ($fieldType == 'date' && (int) $data[$field->name])
			{
				...
				try
				{
					...
					if ($date === false)
					{
						$errors[$field->name] = Text::sprintf(
							'EB_DATE_FIELD_IS_INVALID_FORMAT',
							$field->title,
							$dateFormat
						);
			...
			if ($fieldType == 'datetime' && (int) $data[$field->name])
			{
				...
				try
				{
					...
					{
						$errors[$field->name] = Text::sprintf(
							'EB_DATETIME_FIELD_IS_INVALID_FORMAT',
							$field->title,
							$dateFormat . ' H:M'
						);
		...
		if (file_exists(JPATH_ROOT . '/components/com_eventbooking/helper/custom_validator.php'))
		{
			require_once JPATH_ROOT . '/components/com_eventbooking/helper/custom_validator.php';
		}
		...
		if (!$v->validate())
		{
			foreach ($v->errors() as $fieldName => $errorMessages)
			{
				...
				{
					$errors[$fieldName] = str_ireplace(
						'[FIELD_NAME]',
						$field->title,
						$field->row->validation_error_message
					);
				}
				else
				{
