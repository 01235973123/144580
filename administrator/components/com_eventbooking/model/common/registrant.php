...
use OSSolution\EventBooking\Admin\Event\Registration\AfterRefundRegistrant;
...
class EventbookingModelCommonRegistrant extends RADModelAdmin
{
	...
	protected function addNewRegistration($row, $input, $form)
	{
		...
		{
			$enteredAmount = trim($data['total_amount']);
		}
		else
		{
			$enteredAmount = '';
	...
	protected function updateRegistration($row, $input, $form)
	{
		...
		if (!$app->isClient('site') || $user->authorise(
		...
		if ($published == 3 && $row->published == 0 && $row->amount > 0 && str_contains(
				$row->payment_method ?? '',
				'os_offline'
			))
		{
	...
	public function resendEmail($id)
	{
		...
		if ($row->group_id > 0)
		{
			// Here we need to resend email to group member
			$rowGroupBilling = $this->getTable();
			$rowGroupBilling->load($row->group_id);

			EventbookingHelper::callOverridableHelperMethod('Mail', 'resendGroupMemberEmail', [$rowGroupBilling, $row]);
	...
	public function delete($cid = [])
	{
		...
		if (count($cid))
		{
			...
			$this->deleteRegistrationData($cid);
	...
	/**
	 * Delete data associated registration records
	 *
	 * @param   array  $ids
	 *
	 * @return void
	 */
	public function deleteRegistrationData(array $ids):void
	{
		if ($ids === [])
		{
			return;
	...
	public function checkinRegistrant($id, $checkinAllGroupMembers = false, $checkEventDate = false)
	{
		...
		if ($checkEventDate)
		{
			...
			// Override start date and end date if setup inside event
			if (property_exists($event, 'check_in_start_date') && (int) $event->check_in_start_date)
			{
				$startDate = Factory::getDate($event->check_in_start_date, $timezone);
			}

			if (property_exists($event, 'check_in_end_date') && (int) $event->check_in_end_date)
			{
				$endDate = Factory::getDate($event->check_in_end_date, $timezone);
			}
	...
	private function storeRegistrantTickets($row, $data)
	{
		...
		if (Factory::getApplication()->isClient('site') && !$user->authorise(
				'eventbooking.registrantsmanagement',
				'com_eventbooking'
			))
		{
	...
	public function refund($row)
	{
		...
		$eventObj = new AfterRefundRegistrant(
			'onAfterRefundRegistrant',
			['row' => $row]
		);
		
		Factory::getApplication()->triggerEvent('onAfterRefundRegistrant', $eventObj);
	}
}
