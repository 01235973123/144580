...
class EventbookingModelCommonRegistrants extends RADModelList
{
	...
	protected function buildQueryColumns(DatabaseQuery $query)
	{
		...
		$query->select('tbl.*')
			->select(
				'ev.event_date, ev.event_end_date, ev.ticket_prefix, ev.cancel_before_date, ev.main_category_id, ev.custom_fields, ev.activate_certificate_feature'
			)
	...
	protected function buildQueryWhere(DatabaseQuery $query)
	{
		...
		if ($this->state->filter_category_id && !$this->state->filter_event_id)
		{
			$allCategoryIds = EventbookingHelperData::getAllChildrenCategories($this->state->filter_category_id, true);
			$query->where(
				'tbl.event_id IN (SELECT event_id FROM #__eb_event_categories 
				WHERE category_id IN (' . implode(',', $allCategoryIds) . '))'
			);
		}
		...
		elseif (!$userHasAdminPermission && !$this->state->filter_category_id)
		{
			...
			$query->where(
				'tbl.event_id IN (SELECT event_id FROM #__eb_event_categories WHERE category_id IN (SELECT c.id FROM #__eb_categories AS c WHERE submit_event_access IN ('
	...
	/**
	 * Method to allow filtering registrants for given eventt
	 *
	 * @param   int            $eventId
	 ...
	 * @return void
	 */
	protected function applyEventFilter(DatabaseQuery $query, int $eventId): void
	{
