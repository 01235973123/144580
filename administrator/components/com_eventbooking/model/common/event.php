...
{
	/**
	 * Override $triggerEvents property to support event triggers for events
	 *
	 * @var bool
	 */
	...
	public function initData()
	{
		...
		if (is_file(JPATH_ADMINISTRATOR . '/components/com_eventbooking/data/event_default_data.php'))
		{
	...
	public function store($input, $ignore = [])
	{
		...
		/**
		 * Allow store additional options to params
		 */
	...
	protected function deleteEventThumb($row)
	{
		...
		if (!$total)
		{
			if (is_file(JPATH_ROOT . '/media/com_eventbooking/images/' . $row->thumb))
			{
				File::delete(JPATH_ROOT . '/media/com_eventbooking/images/' . $row->thumb);
			}

			if (is_file(JPATH_ROOT . '/media/com_eventbooking/images/thumbs/' . $row->thumb))
			{
	...
	protected function deleteEventAttachment($row)
	{
		...
		if (!$total && is_file(Path::clean($attachmentsPath . $row->attachment)))
		{
		...
		if ($attachment && $attachment['name'])
		{
			...
			if (in_array(strtolower($fileExt), $allowedExtensions))
			{
				...
				try
				{
					...
					{
						// Todo: Validate the content of the uploaded file for frontend upload
						File::upload($attachment['tmp_name'], $pathUpload . '/' . $fileName);
					}
				...
				{
					$app->enqueueMessage(sprintf('Failed to upload attachment %s', $fileName));
	...
	protected function updateChildrenEvents(
		...
		if (count($children))
		{
			...
			foreach ($children as $childId)
			{
				...
				$rowChildEvent->event_date = HTMLHelper::_(
						'date',
						$rowChildEvent->event_date,
						'Y-m-d',
						null
	...
	protected function setDateTimeFieldsForChildrenEvents($row, $rowChildEvent, $fields = [])
	{
		...
		foreach ($fields as $field)
		{
			if ((int) $row->{$field})
			{
				...
				$rowChildEvent->{$field} = strftime(
					'%Y-%m-%d %H:%M:%S',
					strtotime($rowChildEvent->event_date) + $duration
				);
			}
			else
			{
	...
	{
		$fileExt        = StringHelper::strtoupper(EventbookingHelper::getFileExt($thumbImage['name']));
		...
		if (is_file(
			Path::clean(JPATH_ROOT . '/media/com_eventbooking/images/' . StringHelper::strtolower($thumbImage['name']))
		))
		{
		...
		{
			if (!is_dir(JPATH_ROOT . '/images/com_eventbooking/' . $user->username))
			{
				Folder::create(JPATH_ROOT . '/images/com_eventbooking/' . $user->username);
			}

			if (!is_dir(JPATH_ROOT . '/media/com_eventbooking/images/' . $user->username))
			{
				Folder::create(JPATH_ROOT . '/media/com_eventbooking/images/' . $user->username);
			}

			if (!is_dir(JPATH_ROOT . '/media/com_eventbooking/images/thumbs/' . $user->username))
			{
		...
		{
			Factory::getApplication()->enqueueMessage(sprintf('Failed to upload thumbnail %s', $fileName));
	...
	protected function processSelectImage(&$data)
	{
		...
		if (is_file(Path::clean($sourceImage)))
		{
		...
		else
		{
			Factory::getApplication()->enqueueMessage(
				'The image you selected for this event does not exist. Please check Image field'
			);
		}
	}
	...
	protected function validateEventCustomFields($data)
	{
		...
		try
		{
			...
			$form               = Form::getInstance(
				'ebeventfields',
				JPATH_ROOT . '/components/com_eventbooking/fields.xml',
				[],
				false,
				'//config'
			);
	...
	public function sendRegistrantsList($id)
	{
		...
		if ($filePath)
		{
			...
			EventbookingHelper::callOverridableHelperMethod(
				'Mail',
				'sendRegistrantsListEmail',
				[$row, $filePath, $this->params]
			);
		}
	}
	...
	public function exportRegistrantsOfEventToFile($eventId, $filename = 'registrants_list.xlsx', $excludeStatuses = [])
	{
		...
		JLoader::register(
			'EventbookingModelRegistrants',
			JPATH_ADMINISTRATOR . '/components/com_eventbooking/model/registrants.php'
		);
