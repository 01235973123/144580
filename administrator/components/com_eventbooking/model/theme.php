...
class EventbookingModelTheme extends RADModelAdmin
{
	 ...
	 *
	 * @param   EventbookingTableTheme  $row
	...
	public function install($themePackage)
	{
		...
		try
		{
			File::upload($themePackage['tmp_name'], $destinationDir);
		}
		...
		for ($i = 0, $n = count($files); $i < $n; $i++)
		{
			...
			elseif ($file->getName() == 'folder')
			{
				...
				{
					if (is_dir($themesDir . '/' . $folderName))
					{
		...
		if ($root->media && count($root->media->children()))
		{
			...
			foreach ($files as $file)
			{
				...
				elseif ($file->getName() == 'folder')
				{
					...
					if (is_dir($source . '/' . $folderName))
					{
						if (is_dir($mediaDir . '/' . $folderName))
						{
	...
	public function uninstall($id)
	{
		...
		foreach ($files as $file)
		{
			if ($file->getName() == 'filename')
			{
				...
				if (is_file(Path::clean($themeFolder . '/' . $fileName)))
				{
			...
			elseif ($file->getName() == 'folder')
			{
				...
				if (is_dir($themeFolder . '/' . $folderName))
				{
