...
class EventbookingModelEvent extends EventbookingModelCommonEvent
{
	...
	public function import($file, $filename = '')
	{
		...
		foreach ($events as $event)
		{
			...
			if (!empty($event['image']) && is_file(Path::clean(JPATH_ROOT . '/' . $event['image'])))
			{
				$fileName = File::makeSafe(basename($event['image']));

				if (!is_file(JPATH_ROOT . '/media/com_eventbooking/images/thumbs/' . $fileName))
				{
			...
			// Check and process group rates
			if (!empty($event['group_registration_rates']))
			{
				$groupRatesData = [
					'registrant_number' => [],
					'price'             => [],
				...
				// Now, we parse the rate
				$groupRegistrationRates = explode(';', $event['group_registration_rates']);

				foreach ($groupRegistrationRates as $groupRegistrationRate)
				{
					if (!str_contains($groupRegistrationRate, ':'))
					{
						continue;
					}

					[$numberMember, $rate] = explode(':', $groupRegistrationRate);

					$groupRatesData['registrant_number'][] = $numberMember;
					$groupRatesData['price'][]             = $rate;
				}

				if (count($groupRatesData['registrant_number']) > 0)
				{
					$this->storeEventGroupRegistrationRates($eventId, $groupRatesData, $isNew);
				}
			}
